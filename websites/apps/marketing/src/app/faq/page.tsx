...
const faqs: FAQItem[] = [
  ...
  },
  {
    ...
    a: (
      <div className="space-y-4">
        <p>This is the central challenge of any indexing tool, and SourcePrep takes it more seriously than most. Three mechanisms work together:</p>
        <ol className="list-decimal list-inside space-y-3 text-text-muted">
          <li><strong className="text-text">Incremental re-indexing.</strong> SourcePrep tracks file hashes at the individual symbol level. When you edit a file, only the affected chunks are re-embedded — not the entire project. A real-time file watcher detects changes within seconds.</li>
  ...
  },
  {
    ...
    a: (
      <div className="space-y-6">
        ...
        <div>
          <p className="font-semibold text-text mb-3">What SourcePrep adds on top:</p>
          <div className="overflow-x-auto">
            <table className="text-sm w-full border-collapse">
              <thead>
                <tr className="border-b border-border">
  ...
  },
  {
    ...
    a: (
      <div className="space-y-4">
        ...
        <p>Instead, SourcePrep uses the trace graph as a <strong>navigation structure</strong>:</p>
        <ol className="list-decimal list-inside space-y-2 text-text-muted">
          <li>Semantic search finds the 5 most relevant chunks</li>
          <li>If <code className="bg-surface px-1.5 py-0.5 rounded text-xs border border-border-subtle">trace_expand</code> is on, SourcePrep follows the graph edges to find structurally related code</li>
        ...
        <p className="text-text-muted text-sm italic">Think of it as using Google Maps to find directions vs. printing out every road in the city. SourcePrep reads the map, gives you just the route.</p>
      </div>
    ),
  },
  {
  ...
  },
  {
    ...
    a: (
      <div className="space-y-4">
        ...
        <p>SourcePrep&apos;s approach — find the 5 best chunks, optionally follow structural relationships, and deliver ~2K tokens of high-signal context — aligns with the research recommendation of &quot;Retrieve then Solve.&quot;</p>
      </div>
    ),
  },
  {
    id: "not-enough",
    q: "What if SourcePrep's 5 chunks aren't enough?",
    a: (
      <div className="space-y-4">
  ...
  },
  {
    ...
    a: (
      <div className="space-y-4">
        ...
        <p>However, if strict data privacy is required (e.g., enterprise compliance, air-gapped environments), you can easily connect local models via Ollama. SourcePrep is designed to support both seamlessly. The code <em>index</em> itself always remains 100% local on your machine.</p>
      </div>
    ),
  },
  {
  ...
  {
    id: "semantic-search",
    q: "I already use semantic search tools. How is SourcePrep different?",
    a: (
      <div className="space-y-4">
        <p>Pure semantic search (vector embeddings) excels at fuzzy discovery &mdash; finding code when you describe it in natural language. SourcePrep includes a full semantic search engine (ONNX embeddings, runs locally on CPU, no external service needed).</p>
        ...
        <p>SourcePrep adds a <strong>deterministic dependency graph</strong> on top of embeddings. When the AI asks about blast radius, it gets exact callers and importers from the parsed AST &mdash; not a list of files that happen to mention similar words. And because the graph and the vector index share a token budget, you get both conceptual relevance and structural precision without blowing up your context window.</p>
      </div>
    ),
  },
  {
  ...
  {
    id: "gpu",
    q: "Does SourcePrep require a powerful GPU?",
    a: (
      <div className="space-y-4">
        <p><strong>No.</strong> The core SourcePrep indexing engine, structural graph generation, and ONNX embeddings all run highly efficiently on standard CPUs. You only need a powerful GPU if you choose to run large local reasoning models (like Qwen or Llama 3) via Ollama instead of using cloud APIs.</p>
  ...
  },
  {
    ...
    a: (
      <div className="space-y-4">
        <p><strong>Yes</strong> &mdash; and it handles docs differently than code, which matters. SourcePrep runs two distinct compression strategies: <strong>structural compression</strong> for code files (preserving full source for top results, signatures for medium-relevance, names for peripheral) and <strong>language-aware compression</strong> for Markdown and text (preserving concepts while stripping filler words).</p>
        <p>This means you can include architecture docs, API references, and design decisions in your project scope, and SourcePrep will compress them intelligently alongside your code.</p>
      </div>
    ),
  },
];
