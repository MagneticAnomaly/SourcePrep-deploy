...
const faqs: FAQItem[] = [
  ...
  },
  {
    ...
    a: (
      <div className="space-y-4">
        <p>This is the central challenge of any indexing tool, and Prep takes it more seriously than most. Three mechanisms work together:</p>
        <ol className="list-decimal list-inside space-y-3 text-text-muted">
          <li><strong className="text-text">Incremental re-indexing.</strong> Prep tracks file hashes at the individual symbol level. When you edit a file, only the affected chunks are re-embedded — not the entire project. A real-time file watcher detects changes within seconds.</li>
          <li><strong className="text-text">Automatic observation staleness.</strong> Prep&apos;s Persistent Agent Memory links observations to files. When a file changes, those observations are flagged <code className="bg-surface px-1 rounded border border-border-subtle text-xs">[STALE]</code> — the AI sees them with a clear warning, not as gospel.</li>
        ...
        <p>Most other tools stop at #1. Prep is the only local context engine that extends freshness tracking all the way into the AI&apos;s own learned knowledge.</p>
      </div>
    ),
  },
  {
    ...
    a: (
      <div className="space-y-4">
        <p><strong>No.</strong> Prep&apos;s default output is <strong>~1,500 tokens</strong> (6,000 characters). With trace expansion enabled, it&apos;s ~2,000 tokens.</p>
        <div className="overflow-x-auto">
          <table className="text-sm w-full border-collapse">
            ...
            <tbody className="text-text-muted">
              <tr className="border-b border-border-subtle"><td className="py-2 pr-6">Prep default context</td><td>~1,500</td></tr>
              <tr className="border-b border-border-subtle"><td className="py-2 pr-6">Prep + trace expansion</td><td>~2,000</td></tr>
        ...
        <p>Prep typically consumes <strong>under 1% of your available context window</strong> on any 2026 frontier model. It is designed to be a <em>precision instrument</em>, not a firehose — it sends the 5 most relevant code chunks under a hard character ceiling, not your entire codebase.</p>
      </div>
    ),
  },
  {
    ...
    a: (
      <div className="space-y-4">
        ...
        <p>Prep&apos;s defaults (1,500–2,000 tokens) sit well below every known saturation point.</p>
      </div>
    ),
  },
  {
    ...
    a: (
      <div className="space-y-6">
        <p><strong>Yes — and Prep doesn&apos;t replace that.</strong> All three tools solve the basic problem of &quot;find relevant code and inject it.&quot; Prep uses the same foundational technique (embed → cosine similarity → top-K) for core retrieval. <strong>We are not reinventing that wheel.</strong></p>
        <div>
          <p className="font-semibold text-text mb-3">What Prep adds on top:</p>
          <div className="overflow-x-auto">
            <table className="text-sm w-full border-collapse">
              <thead>
                <tr className="border-b border-border">
                  ...
                  <th className="text-center py-2 px-3 text-primary font-semibold">Prep</th>
                </tr>
              </thead>
              <tbody className="text-text-muted">
                {[
  ...
  },
  {
    ...
    a: (
      <div className="space-y-4">
        ...
        <p>Prep automates the granular, structural layer \u2014 the connective tissue of your codebase. Your rules file handles the human intent. Together, the AI gets both &quot;what the code does&quot; and &quot;what the team wants.&quot;</p>
      </div>
    ),
  },
  {
    ...
    a: (
      <div className="space-y-4">
        ...
        <p>Instead, Prep uses the trace graph as a <strong>navigation structure</strong>:</p>
        <ol className="list-decimal list-inside space-y-2 text-text-muted">
          <li>Semantic search finds the 5 most relevant chunks</li>
          <li>If <code className="bg-surface px-1.5 py-0.5 rounded text-xs border border-border-subtle">trace_expand</code> is on, Prep follows the graph edges to find structurally related code</li>
        ...
        <p className="text-text-muted text-sm italic">Think of it as using Google Maps to find directions vs. printing out every road in the city. Prep reads the map, gives you just the route.</p>
      </div>
    ),
  },
  {
  ...
  },
  {
    ...
    a: (
      <div className="space-y-4">
        ...
        <p><strong>Should you worry?</strong> Less than you think — Prep already mitigates it:</p>
        <ul className="space-y-2 text-text-muted">
          <li><strong className="text-text">Most relevant first.</strong> Prep sorts chunks by descending relevance score. The highest-scoring chunk is at the top — exactly where models pay the most attention.</li>
          <li><strong className="text-text">Small context volume.</strong> At 1,500–2,000 tokens, Prep&apos;s output is short enough that there <em>isn&apos;t</em> a meaningful &quot;middle&quot; to get lost in. The problem primarily affects contexts &gt;10K tokens.</li>
  ...
  },
  {
    ...
    a: (
      <div className="space-y-4">
        ...
        <p>Prep&apos;s approach — find the 5 best chunks, optionally follow structural relationships, and deliver ~2K tokens of high-signal context — aligns with the research recommendation of &quot;Retrieve then Solve.&quot;</p>
      </div>
    ),
  },
  {
    id: "not-enough",
    q: "What if Prep's 5 chunks aren't enough?",
    a: (
      <div className="space-y-4">
  ...
  },
  {
    ...
    a: (
      <div className="space-y-4">
        <p>Prep&apos;s <em>foundation</em> is RAG — embed, search, retrieve. That&apos;s the same technique Cursor, Windsurf, and most modern tools use internally.</p>
        <p>What makes Prep different is what happens <strong>on top of</strong> basic retrieval:</p>
        <ul className="space-y-3 text-text-muted">
          ...
          <li><strong className="text-text">Intent-aware weighting.</strong> Prep detects whether your query is about implementation, debugging, or architecture and adjusts which types of content are prioritized. Automated — not something you configure per query.</li>
          <li><strong className="text-text">User transparency and control.</strong> Every other tool is a black box. Prep shows you the scores, lets you set weights, and tells you exactly what was sent and why.</li>
  ...
  },
  {
    ...
    a: (
      <div className="space-y-4">
        <p><strong>No.</strong> Prep is local-first software &mdash; indexing, embedding, graph construction, search, and compression all happen on your machine. Your code never leaves your filesystem.</p>
        <p>The only exceptions are things you explicitly opt into: if you configure a cloud LLM provider (OpenAI, Anthropic, Google) for the optional enrichment pipeline, only retrieved context snippets are sent to that provider &mdash; never your raw codebase. Prep also makes a single HTTPS call during license activation. Both are optional and auditable.</p>
        <p>What makes this different from tools that <em>claim</em> local-first: Prep ships its own local embedding model (ONNX, runs on CPU) and Rust parser. You don&apos;t need an internet connection, Docker, or external tools like Ollama for the core indexing and search functionality to work out of the box.</p>
      </div>
    ),
  },
  {
    ...
    a: (
      <div className="space-y-4">
        ...
        <p>However, if strict data privacy is required (e.g., enterprise compliance, air-gapped environments), you can easily connect local models via Ollama. Prep is designed to support both seamlessly. The code <em>index</em> itself always remains 100% local on your machine.</p>
      </div>
    ),
  },
  {
    ...
    a: (
      <div className="space-y-4">
        ...
        <p>Prep builds its own persistent trace graph using tree-sitter, a zero-dependency parser that handles 15+ languages from a single binary. The graph persists to disk, loads instantly, and works offline. No runtime, no IDE backend, no configuration per language.</p>
  ...
  {
    id: "semantic-search",
    q: "I already use semantic search tools. How is Prep different?",
    a: (
      <div className="space-y-4">
        <p>Pure semantic search (vector embeddings) excels at fuzzy discovery &mdash; finding code when you describe it in natural language. Prep includes a full semantic search engine (ONNX embeddings, runs locally on CPU, no external service needed).</p>
        ...
        <p>Prep adds a <strong>deterministic dependency graph</strong> on top of embeddings. When the AI asks about blast radius, it gets exact callers and importers from the parsed AST &mdash; not a list of files that happen to mention similar words. And because the graph and the vector index share a token budget, you get both conceptual relevance and structural precision without blowing up your context window.</p>
      </div>
    ),
  },
  {
    ...
    a: (
      <div className="space-y-4">
        <p>Prep is a standalone daemon and an <strong>MCP (Model Context Protocol) server</strong>. It is not locked to any IDE. Today it works with <strong>Cursor</strong>, <strong>Windsurf</strong>, <strong>Claude Code</strong>, and <strong>Claude Code</strong> (terminal). A VS Code extension is in development.</p>
        <p>Because Prep uses the open MCP standard, any future editor or agent that supports MCP will work automatically &mdash; no integration work on our end. Your index, weights, observations, and full project configuration are stored locally and editor-independent. Switch tools without losing anything.</p>
      </div>
    ),
  },
  {
    ...
    a: (
      <div className="space-y-4">
        <p><strong>No.</strong> The core Prep indexing engine, structural graph generation, and ONNX embeddings all run highly efficiently on standard CPUs. You only need a powerful GPU if you choose to run large local reasoning models (like Qwen or Llama 3) via Ollama instead of using cloud APIs.</p>
  ...
  },
  {
    ...
    a: (
      <div className="space-y-4">
        ...
        <p>Prep offers a <strong>Free tier</strong> for evaluation. Pro is a <strong>one-time $79 perpetual license</strong> &mdash; not a subscription. You pay once, own the software, and it works offline forever. We fund development through that, not by monetizing your data or adding telemetry.</p>
      </div>
    ),
  },
  {
    ...
    a: (
      <div className="space-y-4">
        <p><strong>Yes</strong> &mdash; and it handles docs differently than code, which matters. Prep runs two distinct compression strategies: <strong>structural compression</strong> for code files (preserving full source for top results, signatures for medium-relevance, names for peripheral) and <strong>language-aware compression</strong> for Markdown and text (preserving concepts while stripping filler words).</p>
        <p>This means you can include architecture docs, API references, and design decisions in your project scope, and Prep will compress them intelligently alongside your code.</p>
      </div>
    ),
  },
];
      ...
      <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8 py-16">
        ...
        <div className="text-center mb-16">
          ...
          <h1 className="text-4xl font-medium tracking-tight text-text sm:text-5xl mb-4 max-w-xl mx-auto">
            Context, tokens, and how Prep actually works
          </h1>
          <p className="text-lg text-text-muted max-w-xl mx-auto">
            Answers about token budgets, context quality, and how Prep fits alongside the tools you already use.
          </p>
        </div>
