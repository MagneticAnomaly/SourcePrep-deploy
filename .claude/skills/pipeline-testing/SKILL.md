...
|---|---|---|---|
| W1 | Fresh index (empty `.sourceprep/`) | `POST /pipeline/rebuild` on a never-built project | Stages progress 1→15 in order; each group transitions `idle→queued→running→completed`; all 15 manifests written |
