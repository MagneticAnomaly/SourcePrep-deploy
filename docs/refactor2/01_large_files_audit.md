...
1. **`src/prep/core/trace.py`** (2,460 lines)
   ...
   - *Optimization Opportunity*: Move language-specific analyzers into their own files under a `src/prep/core/analyzers/` directory.
...
5. **`src/prep/core/index.py`** (1,926 lines)
...
6. **`src/prep/mcp_server.py`** (1,784 lines)
...
7. **`src/prep/api/routers/trace.py`** (1,606 lines)
...
3. **`src/prep/dashboard/src/hooks/useDashboardPanels.tsx`** (883 lines)
