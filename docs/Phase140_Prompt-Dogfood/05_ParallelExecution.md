...
| B | B4 | ✅ complete (2026-05-19) | all 3 sites: `analysis` | epistemic-doc: same schema-drift + batched-vs-single guidance gap as B3 + needs doc_status reconciliation clause. hr-agents-md: page hypothesis #1 was incorrect (managed markers don't exist in HR code); real risk is edit-preservation literalness. hr-soul-md: page hypothesis #3 was incorrect (first-person voice IS used correctly); real risk is per-section voice drift + whether SOUL.md is consumed at runtime at all. Output capture needed for HR verdicts. |
| B | B5 | ✅ complete (2026-05-19) | all 3 sites: `analysis` (rules-agents-md already had cleanup verdict from prior session) | hr-auto-roles: corrects page schema (5 fields not 4; no mcp_tools field — tool assignment lives elsewhere); audit_findings integration is the strongest grounding hook; recommends hard-constraint role-count + display_name NAMING RULES. custodian-safety: corrects page schema (UPPER_SNAKE_CASE; no `archive` value); conservatism is engineered via 6-question structure (not just persona-tone); KEEP path is in schema but unspecified in decision logic; recommends REASON RULES. Page stubs need correction in both cases (they were written from intent, not actual code). |
| A | A1 | ✅ complete (2026-05-19) | all 3 sites: `analysis` | atlas-single-doc: 5 sections required but only 3 emitted on PowerMate (FLOW + CROSS-CUTTING silently dropped under 1560-char budget cap). Quality of 3 emitted is excellent. Proposed all-sections-present clause. atlas-root + atlas-segment: structural review only (no multi-segment baseline). Page stub for atlas-segment was wrong about section count (says 4, actually 6) — corrected in iteration. |
| A | A2 | ✅ complete (2026-05-19) | all 3 sites: `analysis` | audit-summary: well-engineered; Module Status truncates to 8 of 20 modules (instruction ambiguous); page hypothesis #2 corrected (uses A-F grade not 0-100 score). audit-architecture: well-engineered; 5 boundary violations identified; module-identifier casing mixed (kebab vs snake vs Title Case) in output. audit-gaps: well-engineered structure but GAP-1 flags 11 "dead-code" files as Critical — false positive because grounding lacks inferred-edges; cross-checking against batch-edges output shows all 11 are runtime-instantiated. Same grounding-shape pattern as concept-pipeline-grounding-gap finding. |
| A | A3 | ✅ complete (2026-05-19) | audit-inventory: `partial` (shipped). audit-tech-debt: `partial` (shipped). batch-symbol: `analysis`. | audit-inventory: 57% preamble + 3× table regeneration (529 lines, real content starts at line 301). audit-tech-debt: 80% preamble (580 lines, real content starts at line 465). Shipped anti-preamble clause on all 5 audit SYSTEM prompts (commit 76d0b119). batch-symbol: 64% of records are fallback stubs (not LLM output); role taxonomy too narrow for Swift; defer iteration until full coverage. Page stub correction: audit-inventory said "only 9 lines" — actually 529. |
| A | A4 | ✅ complete (2026-05-19) | all 3 sites: `analysis` | Major snapshot-methodology finding: all 4 batch-* snapshot files (batch-file/doc/narrative/symbol) are byte-identical — the catalogue augmenter produces ONE mixed jsonl, capture-notes copied it into 4 dirs. batch-file: 100% LLM coverage on 18 file records; related_files has ~25-40% questionable cases (port batch-edges EVIDENCE DISCIPLINE). batch-doc: doc_status 100% "active" (clumping); research files classified as "overview" instead of more-specific "research" taxonomy value. batch-narrative: page stub schema corrected (7 fields not 2); cannot separately audit from batch-doc. |
| A | A5 | ✅ complete (2026-05-19) | all 3 sites: `analysis` (no PowerMate output — Researcher agent never run) | Major page-stub correction: ALL THREE researcher page stubs had output schemas drafted from imagination, not from code. Corrections shipped per site. researcher-topic: JSON discipline good; max_topics parameterization already addresses open question. researcher-research: outputs Markdown not JSON; HIGH preamble-leakage risk (same shape as A3 audit prompts before fix). researcher-plan: outputs 5 flat fields not nested steps; effort+risk enums lack passing-test definitions. |
