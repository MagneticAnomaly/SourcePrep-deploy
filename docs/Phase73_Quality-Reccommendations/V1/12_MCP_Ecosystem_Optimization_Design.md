...
> Date: 2026-04-05 | Aligning Prep's MCP primitives with Claude Code's ecosystem for native integration

---

## 1. Vision

Prep currently exposes almost all capabilities through a single MCP primitive (tools). The MCP spec defines three primitives with distinct control models. By placing each capability on the right primitive, Prep becomes a native part of every MCP client's ecosystem — not a bolted-on tool collection.
...
| Primitive | Who Initiates | Best For | Prep Examples |
|-----------|--------------|----------|----------------|
...
| Resource templates (`prep://modules/{name}`) | Not implemented | `resources/templates/list` + resolution |
...
| Tool | Purpose | Change |
|---|---|---|
| `prep` | Ambient structural context | **Slimmed** — orientation header + routing. Resources carry heavy reference data. Remains the "call at task start" entry point. |
| `prep_search` | Semantic search + trace expansion | **Unchanged** |
| `prep_impact` | Dependency/blast radius analysis | **Unchanged** |
| `prep_observe` | Cross-session memory (save/get) | **Unchanged** |

**Consolidation candidates:**
- `prep_audit` read actions → resource (`prep://audit`). Scan-on-demand remains a tool action (either kept as lean tool or folded into existing tool with an `action` parameter).
- `prep_concepts` read → resource (`prep://concepts`). Save action → fold into `prep_observe` or keep as lean tool.

**New annotations on all tools:**

```python
# Example for prep tool
{
    "name": "prep",
...
| Annotation | prep | search | impact | observe |
|---|---|---|---|---|
| `title` | "Prep: Codebase Context" | "Prep: Code Search" | "Prep: Impact Analysis" | "Prep: Observations" |
...
|---|---|---|---|
| `prep://{pid}/atlas` | Full codebase atlas (identity, stack, workspace map, cross-cutting) | Index rebuild / atlas regen | assistant |
| `prep://{pid}/structure` | Hub files list with edge counts, structural role descriptions | Index rebuild | assistant |
| `prep://{pid}/modules` | Module list with file counts, dependencies, summaries | Index rebuild | assistant |
| `prep://{pid}/modules/{name}` | Single module detail (**template resource**) | Index rebuild | assistant |
| `prep://{pid}/audit` | Latest audit findings summary | Audit scan completion | both |
| `prep://{pid}/concepts` | Epistemic knowledge layer — categories + top concepts | Concept save/approve | assistant |
| `prep://{pid}/focus` | User's selected focus areas with content excerpts | User config change | assistant |
| `prep://{pid}/health` | Index freshness, coverage, build status | Build events | both |

**Design decisions:**
- **Tier-adaptive content**: Resource responses respect the client's context budget. Claude Code reading `@prep://hubs` gets 10 hubs at LOD 0. Cline gets 4 at LOD 2. Same URI, different fidelity.
- **Resource templates**: `prep://{pid}/modules/{name}` enables auto-complete when users type `@prep://modules/`. Requires `resources/templates/list` handler.
...
|---|---|---|
| `prep-onboard` | none | Multi-message: system orientation + embedded `prep://atlas` + suggested first actions |
| `prep-review` | `file_path` (required), `scope` (optional: file/module/blast-radius) | Impact context + review checklist + embedded hub resources for related files |
| `prep-investigate` | `query` (required) | Search strategy + trace expansion context + module map for relevant area |
| `prep-health` | `focus` (optional: debt/complexity/coverage) | Embedded audit findings + prioritized recommendations |
| `prep-plan` | `change` (required) | Impact analysis + affected files + change strategy (keep existing, refine) |

**Design decisions:**
- **Prompts embed resources** via `resource` content type (e.g., `onboard` embeds `prep://atlas` and `prep://modules`). This means prompt content stays fresh without duplication.
...
- **`.claude/skills/prep.md` remains** as a Claude Code-specific enhancement. Skills can restrict tool access and delegate to subagents — capabilities prompts lack. The skill and prompts serve different audiences: skill for Claude Code power users, prompts for all MCP clients.
...
Embed a hash of the atlas content in CLAUDE.md. The MCP server computes the current hash on each `prep` call and compares.

**In CLAUDE.md** (written by `rules_generator.py`):
```markdown
<!-- prep-managed-start -->
<!-- prep-atlas-hash:a3f2b7c1e9d4 -->
# Prep Integration
prep_project_id: 1d6f0b35-...
...
## Codebase Atlas
[full atlas content]
<!-- prep-managed-end -->
...
1. On `prep` call, compute `hashlib.sha256(current_atlas).hexdigest()[:12]`
...
**Existing marker system is safe.** `rules_generator.py:682-698` already uses `<!-- prep-managed-start/end -->` markers and only replaces the managed section. All user content and content from other tools is preserved.
...
- `@prep://structure` in Claude Code → 10 hubs at LOD 0 (~15K chars)
- `@prep://structure` in Cline → 4 hubs at LOD 2 (~4K chars)
...
| Document `"allow": ["mcp__prep"]` in AGENTS.md | `rules_generator.py` | Small | Users stop clicking approve on every call |
...
| Atlas content hash | `rules_generator.py` | Compute SHA-256 of atlas, embed as `<!-- prep-atlas-hash:... -->` |
...
| Permission hint in AGENTS.md | `rules_generator.py:_build_managed_content()` | Add `"allow": ["mcp__prep"]` recommendation |
...
| Resource template support | `server.py` | Add `resources/templates/list` handler for `prep://modules/{name}` |
...
| Add new prompts | `server.py:_PROMPTS` + `handle_prompts_get()` | `prep-investigate`, `prep-health` |
| Resource embedding in prompts | `server.py:handle_prompts_get()` | Add `resource` content type referencing `prep://` URIs |
| Argument auto-completion | `server.py` | Add `completion/complete` handler for file_path and query args |
| Update skill to reference prompts | `.claude/skills/prep.md` generator | Note prompt availability alongside tools |
...
|---|---|---|---|---|---|---|
| Ambient context / atlas | `prep` (slim) | `prep://atlas` | embedded in onboard | overview | /atlas | Architecture panel |
| Semantic search | `prep_search` | — | embedded in investigate | search | /search | Search panel |
| Impact analysis | `prep_impact` | — | embedded in review | — | /trace/impact | Trace panel |
| Hub files | — | `prep://structure` | — | coverage | /trace/hub_files | — |
| Module map | — | `prep://modules` | — | — | /modules | — |
| Single module detail | — | `prep://modules/{name}` | — | — | /modules/{id} | — |
| Audit findings | — | `prep://audit` | embedded in health | audit | /audit/findings | Audit panel |
| Audit scan | `prep_audit` (scan only) | — | — | audit scan | POST /audit | Dashboard |
| Observations | `prep_observe` | — | — | — | /observations | Observations |
| Concepts (browse) | — | `prep://concepts` | — | — | /concepts | Concepts |
| Concepts (save) | `prep_observe` or dedicated | — | — | — | POST /concepts | Create concept |
| Focus areas | — | `prep://focus` | — | — | /included_paths | Settings |
| Index health | — | `prep://health` | embedded in health | status | /status | Status |
| Onboard workflow | — | — | `prep-onboard` | — | — | — |
| Review workflow | — | — | `prep-review` | — | — | — |
| Investigate workflow | — | — | `prep-investigate` | — | — | — |
| Health check workflow | — | — | `prep-health` | — | — | — |
| Plan change workflow | — | — | `prep-plan` | — | — | — |

---

## 10. Key Design Decisions

### 10.1 Why keep `prep` as a tool AND have resources?

The `prep` tool serves a unique role: it's the "call at task start" entry point that agents invoke autonomously. No resource can do this — resources require user action (`@` mention). The tool returns a slim orientation response; resources carry the heavy reference data. Different access patterns, same underlying data.

### 10.2 Why not merge prompts into Claude Code skills?

Skills are Claude Code-only. MCP prompts work in Cursor, Windsurf, Gemini, VS Code, Copilot — every MCP client. One implementation serves all. The `.claude/skills/prep.md` remains for Claude Code power users who need skill-specific features (subagent delegation, tool restriction).

### 10.3 Why tier-adaptive resources?

The same `@prep://structure` URI should work for a 1M-context Opus user and a 20K-context local model user. Returning 15K chars to a 20K budget client is unusable. Tier-adaptive content means resources "just work" regardless of client capability.
...
2. **All 5 prompts available** as `/mcp__prep__*` slash commands across MCP clients
3. **Tool count stays at 4** (prep, search, impact, observe) — lean and focused
4. **Atlas hash eliminates duplication** — `prep` response omits atlas when CLAUDE.md is current
