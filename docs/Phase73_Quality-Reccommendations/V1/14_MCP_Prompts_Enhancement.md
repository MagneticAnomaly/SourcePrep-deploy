...
> Date: 2026-04-05 | Expanding Prep's MCP prompts from 3 to 5 structured workflows

---

## 1. Motivation

Prep's MCP prompts (exposed via `prompts/list` and `prompts/get`) are the user-initiated workflow templates that MCP clients surface as slash commands (e.g., `/prep-review` in Claude Code, Cursor, etc.). They prime conversations with structured multi-step instructions that guide agents through Prep's tools in a coherent sequence.

The original set of 3 prompts (`prep-analyze`, `prep-review`, `prep-plan`) covered architecture analysis, code review, and change planning. However, two common workflows were missing:

1. **Onboarding** — A first-contact orientation prompt for agents (or humans) new to a codebase. The old `prep-analyze` was close but framed as "analyze the architecture" rather than "orient me to this codebase." The distinction matters: onboarding should produce a mental model, not a critique.
...
3. **Health check** — A codebase health prompt that leverages `prep_audit` findings and structural context to produce prioritized, actionable recommendations. The old prompts had no audit integration at all.

Additionally, the existing `prep-review` prompt lacked a `file_path` argument, making it unclear which file should be reviewed. The new version requires `file_path` and makes `scope` optional with clear values (`file`, `module`, `blast-radius`).
...
**Before:** 3 prompts — `prep-analyze`, `prep-review`, `prep-plan`

**After:** 5 prompts — `prep-onboard`, `prep-review`, `prep-plan`, `prep-investigate`, `prep-health`

| Prompt | Arguments | Purpose |
|--------|-----------|---------|
| `prep-onboard` | (none) | Orient to codebase: structural overview, hub files, entry points, attention areas |
| `prep-review` | `file_path` (required), `scope` (optional) | Structural code review with blast radius awareness |
| `prep-plan` | `change` (required) | Impact-aware change planning with dependency ordering |
| `prep-investigate` | `query` (required) | Deep-dive into a topic: search, trace, module context, mental model |
| `prep-health` | `focus` (optional) | Codebase health check with audit findings and prioritized fixes |

Key changes to existing prompts:
- **`prep-analyze` removed from the list** but retained as backward-compatible alias in `handle_prompts_get` (redirects to `prep-onboard`)
- **`prep-review` gained `file_path`** as a required argument, making it explicit which file is being reviewed
- **`prep-plan` description updated** from `--` to `—` for typographic consistency
...
1. **Each prompt leads with a Prep tool call.** The first step in every workflow is a Prep MCP tool call (`prep`, `prep_search`, `prep_impact`, or `prep_audit`). This ensures the agent gets structural context before reasoning.
...
3. **Tool calls are explicit.** Instead of saying "understand the dependencies," the templates say "Call `prep_impact` on the file." This reduces ambiguity and increases the chance that agents actually use the tools.

4. **`prep-investigate` leads with search, not overview.** Unlike other prompts that start with `prep` for structural context, investigate starts with `prep_search` because the user has a specific query. Overview comes second to provide structural framing around the search results.

5. **`prep-health` integrates `prep_audit`.** This is the only prompt that explicitly invokes the audit tool. The optional `focus` argument lets users narrow to debt, complexity, coverage, or architecture.

6. **Backward compatibility for `prep-analyze`.** The old prompt name recursively calls `handle_prompts_get` with `prep-onboard` as the name, preserving any integrations that reference the old name.
...
This replaces `prep-analyze`. The framing shift from "analyze" to "orient" changes the agent's output from a critique to a navigational map. Step 5 optionally pulls in audit findings, bridging the onboard and health workflows.

#### prep-review

```
Review `{file_path}` (scope: {scope}) using Prep's structural understanding.
...
```
Check the health of this codebase using Prep.{focus_text}
...
| File | Change |
|------|--------|
| `src/prep/mcp/server.py` | Replaced `_PROMPTS` (3 prompts -> 5), rewrote `handle_prompts_get` (3 branches -> 6 including backward compat) |
...
| `test_prompt_onboard_returns_messages` | `handle_prompts_get` for `prep-onboard` returns a valid message structure with `role: "user"` |
| `test_prompt_investigate_uses_query` | `handle_prompts_get` for `prep-investigate` interpolates the `query` argument into the message text |
| `test_prompt_analyze_backward_compat` | `handle_prompts_get` for the old `prep-analyze` name redirects to `prep-onboard` content (contains "Orient") |
...
|----------------|----------------|
| `prep-onboard` — none | Implemented, returns 5-step orientation workflow |
| `prep-review` — `file_path` (required), `scope` (optional) | Implemented with `file`, `module`, `blast-radius` scope values |
| `prep-plan` — `change` (required) | Retained from previous version, unchanged |
| `prep-investigate` — `query` (required) | Implemented, search-first workflow with mental model deliverable |
| `prep-health` — `focus` (optional) | Implemented with audit integration and top-3 recommendation limit |
...
- `.claude/skills/prep.md` parity — skills and prompts serve different audiences per design doc
...
|--------|-------------------|
| Claude Code | `/prep-onboard`, `/prep-review`, etc. in the slash command menu |
