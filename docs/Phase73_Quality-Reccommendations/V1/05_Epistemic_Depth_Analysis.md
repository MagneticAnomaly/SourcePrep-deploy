...
This document continues the critical epistemic audit by tracing the deep backend logic governing `codrag_audit` staleness and `codrag_search` chunking strategies.
...
### The Symptom
The `codrag_audit` tool repeatedly flags lockfiles like `package-lock.json` as "large files" with generic advice to "split into a subpackage", despite the codebase (`src/codrag/core/audit/analyzers/large_files.py`) containing an explicit filter:
...
In `src/codrag/mcp/server.py` (`tool_audit`), the MCP tool fetches findings using this sequence:
...
### The Fix
Modify `codrag_audit` to accept a `force_refresh` parameter (translating to `synthesize=True` or an explicit fresh endpoint call), or implement backend cache invalidation when the CoDRAG engine version changes. As a quick fix, changing the `codrag_audit action="scan"` logic to always trigger a fresh run for AI calls guarantees accurate context.
...
### The Symptom
`codrag_search` systematically fails to retrieve large, architectural files like `orchestrator.py` because the query isn't matched against the file's holistic concept.
...
2. **Blind Slicing**: Normal files (like the 100KB `orchestrator.py`) are passed to `chunk_code` (`src/codrag/core/chunking.py:208`). 
...
```text
Path: src/codrag/core/orchestrator.py
Hash: abc1234
