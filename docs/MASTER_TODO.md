...
- [x] **P127-F1: Anti-stale soft-hold TTL / cleanup** — **Closed by `fc3d8124`** (Lane C, merged in `57a4e335`). New `PipelineScheduler.sweep_stale_holds(grace_s=300.0)` with backing-state predicate `_hold_has_backing_state`. Wired into the 30s drain-timer callback. Known limitation: drain timer only fires during swarm windows, so orphan holds created in idle periods get cleaned at next swarm-window opening. Tests: `test_stale_hold_sweep.py` 8/8.

- [x] **P127-F2: Pre/post-swarm LLM-direct sites bypass soft-holds** — **Closed by `7477f3de`** (Lane C). 14 sites guarded with `if hold_paused_for_llm(...): raise_hold_paused_for_llm(...)` + `except HoldPausedError: raise` carve-outs. **`CodebaseAtlas.__init__` now accepts `project_id`** and threads it into the atlas `_run_swarm` dispatch. Test: `test_llm_direct_sites_hold_guarded.py` 14/14 parametrized AST contracts (post-merge F2_SITES re-anchored — line numbers shifted by Phase 141 + Lane A atlas fix).

- [x] **P127-F3: Headless runner / `/trace/*` routes** — **Closed by `267b2e3f` + `8d19c308`** (Lane C). The literal `project_id=None` was already gone; bug had shifted to *omitting* the kwarg so it defaulted to None. Three constructors at `enrichment.py:97 / 442 / 676` now pass path-param `project_id` through. `headless_runner.py` deliberately keeps `project_id=""` (single-project, never enters multi-project priority queue). Test: `test_trace_routes_pass_project_id.py` 1/1 AST contract.
...
- [x] **P127-F5: DeepeningLoop hold-handling integration test** — **Closed by `5dcc0f22`** (Lane C). `_FakePausingEnricher` mocks `HoldPausedError` mid-batch; asserts `loop.run()` returns `DeepeningResult` without raising, `iterations == 1`, `_write_epistemic` was called with partial work, `deepening_complete` progress beacon did NOT fire. Covers both sequential and threaded branches. Test: `test_deepening_hold_pause_integration.py` 3/3.

- [x] **P127-F6: Helper consolidation** — **Closed by `671cb2ef`** (Lane C). New `HoldAwareMixin` in `services/pipeline/holds.py` exposes `_hold_paused` / `_raise_hold_paused`. `EpistemicEnricher`, `TraceAugmenter`, `SwarmOrchestrator` all inherit it; `SwarmOrchestrator` overrides `_hold_llm_for_check` to gate against `self.worker_llm`. Net ~70 LoC removed, no behavior change. `test_soft_hold_primitive.py` 21/21 still green.

- [ ] **P127-F7: Lock-discipline gaps on read paths in `scheduler.py`.** `get_priority`, `priority_project_id`, `priority_level`, `priority_projects`, `is_swarm_window_active` read shared dicts WITHOUT `self._lock`. CPython per-key dict reads are atomic so corruption is unlikely, but for consistency wrap in `with self._lock:`. Pure cleanup. **Lane C did NOT touch — still open.**

- [~] **P127-F8: Single-project regression smoke test.** **In progress 2026-06-01.** User running full pipeline on PowerMate against the merged code (Lane C + Phase 141 + Lane A atlas swarm fix) as the live integration gate. Daemon was restarted (PID 31964 → 6881) so merged code is loaded. fast_sync completed (`run-a45bb0e55660`); deep_enrichment in progress as of pipeline status check.

**Lane C merge state (2026-06-01):** All 7 closed F-items + F-7-still-open above. Merge commit `57a4e335` on `main`. Single conflict at `atlas/generator.py` swarm-construction kwargs resolved by combining Phase 141's dynamic `max_wall_time_s=wall_budget` with Lane C's `project_id=self.project_id or None`. 73 cross-lane tests + 172 wider sweep all green. Worktree at `.claude/worktrees/phase-127-129-hygiene/` is now merged and safe to delete (gated on user signal).

**Phase 129 DevLeak Audit — closed:** 55 logger sites rewritten across 11 modules. Regression guard `test_phase129_dev_leak_regression.py` AST-scans `logger.*` positional string literals for `Phase N` / `F-NN` prefixes; `CLEAN_MODULES` list grows monotonically. 11/11 parametrized contracts green.
...
- [ ] **P122-T-swarm_optimizer [KEEP-DORMANT → port-then-decide]:** `src/prep/core/swarm_optimizer.py` (34 LoC) — file is not on the live swarm path; Phase 82 explicitly pruned its `PLAN_TIER_CONCURRENCY` and `PlanTier` (commit `214f261f`, per `Phase82_CloudPipelineConcurrency/05_Completion_Plan.md`:24). The 4 remaining constants (`KIMI_MAX_BATCH`, `GEMINI_MAX_BATCH_ITEMS`, `GEMINI_ATTENTION_QUALITY_CEILING_TOKENS`, `GEMINI_HARD_CONTEXT_TOKENS`) have **0 callers** — the *values* `200_000`/`10` are inlined at `src/prep/core/batch_profiles.py:276,320` and `src/prep/core/epistemic_enrichment.py:588`. **Precondition for deletion:** port the calibration rationale prose in the file's comments to those inline sites, OR have those sites re-import from this file (recentralization). Live swarm files are unaffected (`swarm_orchestrator.py`, `swarm_registry.py`, `concept_generate_swarm.py`, `concept_validate_swarm.py`, `swarm_event_logger.py`, `swarm_models.json`) — verified 2026-05-13. Owner: unassigned. **High-care file: do not delete without owner signoff.**
- [ ] **P122-T-budget_enforcement [KEEP-DORMANT, post-MVP]:** `src/prep/core/budget_enforcement.py` (218 LoC) — Phase 06 enterprise admin feature (`EA-H5` per `docs/Phase06_Team_And_Enterprise/ENTERPRISE_ADMIN_DESIGN.md`). Tested in isolation at `tests/test_pipeline_budget.py:30`; never wired into pipeline runner, audit_log, or dashboard. `cost_estimation.py:6` mentions it in a docstring but does not import. **Enterprise tier is not in MVP scope** so deferral is by-design. Wire when the enterprise tier becomes scope (pipeline runner calls `check_budget()` pre-run + audit_log feeds usage + dashboard admin panel consumes `BudgetStatus`). Alternative: downgrade to DEPRECATE if the tier is shelved permanently. Owner: unassigned.
