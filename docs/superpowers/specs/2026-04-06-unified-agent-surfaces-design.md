...
> Scope: Frontend surfaces only — Prep Dashboard panels, Paperclip Plugin UI, and the interface boundary between them
> Principle: **Prep configures and computes. Paperclip operates and displays. No redundancy.**

---

## 1. Problem Statement

Prep has a mature agent backend — three engines (HR, Researcher, Custodian), a collaboration infrastructure (4 stores, 7 REST endpoints, 3 MCP resources, 3 MCP prompts), and a Paperclip plugin with 5 tools and 4 data providers. The backend is coherent and well-documented.

The frontend surfaces are not. Two UI surfaces exist — the Prep dashboard (React at :5174) and the Paperclip plugin UI (4 declared slots) — with unclear responsibilities and partial implementations:

- The Prep dashboard has 12 agent components across 2 panels (`agent-ops`, `agent-scope`) that expose both **configuration** (scope editing, role generation) and **operational status** (run counts, research history, employee badges). The operational parts overlap with what Paperclip should show.
...
- No surface shows **what Prep actually pushed** to Paperclip — the enrichment pipeline (consensus scores, structural complexity, delta pushes) exists in the backend plan but has no UI home.
- The three Prep engines run and produce findings, but there's no clear flow for how those findings become visible work in Paperclip.

**Goal:** Define what each surface shows, what it doesn't show, and how recommendations flow from Prep engines through to Paperclip tickets — with the simplest possible frontend.
...
Prep agents are **recommendation engines**, not task managers. They analyze the codebase, produce findings, and push them to Paperclip as enriched issues. Paperclip manages the lifecycle (assignment, execution, completion). There is no bidirectional sync — flow is one-directional:

```
Prep engines → enriched issues → Paperclip issue tracker
                                    ↓
                              Paperclip agents → pick up work
                                    ↓
                              call prep:* tools for structural context during execution
```

### 2.2 Three Pipes

All Prep-to-Paperclip communication uses three pipes:
...
| **Runtime context** | Structural intelligence during agent execution | Plugin tools (`prep:context`, `prep:search`, etc.) | Yes — 5 tools in plugin worker |

### 2.3 The Line Between Surfaces

| Responsibility | Prep Dashboard | Paperclip Plugin |
|---------------|------------------|------------------|
...
**The rule:** If Paperclip has a native UI for it, Prep doesn't show it. Prep shows only what you need to **configure the intelligence** and **verify it's working**.

---

## 3. Prep Dashboard — Minimal Configuration Surface

### 3.1 What Stays

**Agent Knowledge Scopes panel** (`agent-scope`) — This is pure Prep configuration. No equivalent in Paperclip. The panel lets users:
...
The existing MCPConnectionCard, unchanged. Shows whether the Prep skill is installed in Paperclip, with install/uninstall/refresh actions.

**Section 3: Push Settings (new, minimal)**

Configuration for how Prep pushes to Paperclip:
...
The plugin has 5 tools (working), 4 data providers (working), 2 actions (working), 1 job (working), and 4 UI slots (thin implementations). The plugin worker (`395 lines`) is solid — it proxies all calls to the Prep daemon correctly.
...
**Enhanced:** This tab shows the **read-only view** of an agent's Prep knowledge scope. It needs two things to work:

1. **Role mapping:** When Prep pushes an agent to Paperclip via `create_agent()`, it should store the Prep role slug in the agent's `adapterConfig.prep_role`. The plugin's data provider reads this instead of looking up state.

2. **File list:** The data provider calls `GET /projects/{pid}/agent-scope/{role}` (already exists) with the mapped role.

The tab shows:
- The role name and how it was resolved (from `adapterConfig.prep_role`)
- A read-only file list (the configured scope paths)
- Active claims on files within the scope (from `agent-claims` data provider)
- A note: "Configure scopes in the Prep dashboard" with a link/instruction

**This tab does NOT allow scope editing** — that's the Prep dashboard's job. Read-only prevents two editing surfaces for the same data.

### 4.4 UI Slot: Issue Context Tab (`codebase-context`, on issues)

**Current:** 42 lines of placeholder.

**Enhanced:** When viewing a Paperclip issue that was pushed from Prep (identifiable by `<!-- prep-address:... -->` in the description), this tab shows the **structural context** behind the issue:
...
- **Related observations** from Prep agents (via `prep_search` with the file paths mentioned in the issue)
- **Impact analysis** for affected files (via the `prep:impact` tool)

For issues NOT pushed from Prep, the tab shows a simpler view:
- "No Prep context for this issue. Use the Enrich action to add structural analysis."
- A button that triggers `prep:context` + `prep:impact` for files mentioned in the issue description

**Data source:** The tab parses the issue description for `prep-address` metadata. If found, it calls the daemon for the specific context. If not, it offers on-demand enrichment.

### 4.5 UI Slot: Settings Page (`prep-settings`)

**Current:** 55 lines of basic config.

**Enhanced:** This is the plugin-side mirror of Prep connection settings:
- Daemon URL (with health check indicator)
- Project ID (auto-detected or manual)
- Auto-context toggle (attach Prep context to new issues automatically)
- Push settings link: "Configure push behavior in the Prep dashboard"

**This page does NOT duplicate the Prep dashboard's push settings.** It only controls plugin-side behavior (what the plugin does with data it receives).
...
**`enrich-issue`** — Calls `prep:context` and `prep:impact` for files mentioned in an issue, then appends structural context to the issue description. Triggered from the Issue Context tab's "Enrich" button. Uses existing daemon endpoints, no new backend work.
...
This section documents how Prep engine findings become enriched Paperclip issues. The backend implementation is specified in `emergence_and_p3_design.md` — this section describes the **flow** and how it connects to the frontend surfaces.
...
Step 5: Visible in Paperclip
  ...
  ├── Issue Context tab shows full Prep analysis
...
The Prep dashboard's push settings control this:
...
| **Prep dashboard monitoring** | Run history, agent heartbeats, task assignment — all belong in Paperclip. Prep shows "last ran 2h ago, pushed 3 issues" and nothing more. |
| **Bidirectional sync** | Prep pushes TO Paperclip. It never pulls back. If we need outcome data later (was the issue accepted?), we poll Paperclip's API — we don't build a sync system. |
| **Duplicate issue views** | Prep dashboard does NOT show a list of pushed issues. That's Paperclip's issue list. Prep shows only the push count summary. |
| **Agent lifecycle management** | Pause/resume/assign agents is Paperclip's job. Prep only creates/updates agents via the staffing pipe. |

---

## 8. Component Inventory — What Changes

### 8.1 Prep Dashboard Components (`packages/ui/src/components/agents/`)
...
### 8.2 Paperclip Plugin UI (`packages/paperclip-plugin-prep/src/ui/`)
...
| `IssueContextTab.tsx` | 42 lines placeholder | Structural context for Prep-pushed issues, on-demand enrichment for others |
| `SettingsPage.tsx` | 55 lines basic config | Add push settings link, verify health check works |

### 8.3 Plugin Worker (`packages/paperclip-plugin-prep/src/worker/index.ts`)

| Change | Description |
|--------|-------------|
| `codebase-health` data provider | Extend to fetch delta + consensus + push summary in parallel |
| `agent-knowledge-scope` data provider | Fix role mapping: read `adapterConfig.prep_role` instead of state lookup |
...
4. **Claims in Prompts** — extend `prep-enrich` and `prep-handoff` prompts
...
13. **IssueContextTab** — implement structural context display for Prep-pushed issues + on-demand enrichment
...
1. **Prep dashboard shows config only** — no run history, no employee lists, no operational monitoring. Engine controls, scope editor, push settings, Paperclip connection.
...
5. **No redundancy** — nothing visible in both Prep dashboard AND Paperclip plugin. Each datum has one home.
6. **Agent scopes editable in Prep only** — Paperclip shows read-only scope view. Configuration happens in one place.
...
| `strategic_direction.md` | Architecture: Prep computes, Paperclip acts | **This spec follows the same principle** for all frontend decisions |
...
2. **Role mapping on agent create** — When `push_to_paperclip()` creates an agent, does the current `PaperclipClient.create_agent()` support `adapterConfig.prep_role`? Need to verify the Paperclip API accepts arbitrary adapter config keys.
...
**Files:** `src/prep/api/routers/collaboration.py`, `src/prep/dashboard/src/hooks/useDashboardPanels.tsx`
...
**Files:** `src/prep/api/routers/agents.py`, possibly new adapter functions in each engine.
...
**Files:** `src/prep/adapters/push_engine.py` or `src/prep/api/routers/agents.py`
...
**Files:** `src/prep/api/routers/projects/watch.py` or `src/prep/core/watcher.py`
...
**Effort:** Trivial — UI-only change in `packages/paperclip-plugin-prep/src/ui/SettingsPage.tsx`.
