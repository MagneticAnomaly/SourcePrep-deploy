...
- Team setups with a centralized Prep server serving multiple users
...
- API: `src/prep/api/routers/trace_routes/enrichment.py:314`
...
3. As a one-shot CLI: `prep prune --orphans` for users with already-corrupted
...
Prep unconditionally excludes AI agent instruction files from tracing
because they are already available to the AI via direct injection (CLAUDE.md
is loaded into the system prompt, AGENTS.md is auto-generated by Prep,
...
`src/prep/core/repo_profile.py` defines two related constants:
...
| File | Line | How |
|------|------|-----|
| `src/prep/core/trace/builder.py` | 100 | `TraceBuilder.__init__` extends `exclude_globs` |
| `src/prep/core/trace/coverage.py` | 93-99 | Coverage always appends them (Phase 89 comment) |
| `src/prep/core/index.py` | 286 | Index building extends `exclude_globs` |
