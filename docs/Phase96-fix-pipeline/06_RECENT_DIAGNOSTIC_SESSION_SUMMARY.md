...
### Problem Statement
The dashboard UI was experiencing general sluggishness, traced to unbounded growth of pipeline log files in `.codrag/logs/` directory.
...
import json
count_nodes = sum(1 for _ in open('.codrag/trace_nodes.jsonl'))
count_epistemic = sum(1 for _ in open('.codrag/trace_epistemic.jsonl'))
print(f"Nodes: {count_nodes}, Enriched: {count_epistemic}, Gap: {count_nodes - count_epistemic}")

# Load and compare node IDs
nodes_ids = {json.loads(l)['id'] for l in open('.codrag/trace_nodes.jsonl')}
epistemic_ids = {json.loads(l)['node_id'] for l in open('.codrag/trace_epistemic.jsonl')}
