...
This guide provides step-by-step instructions for configuring Lemon Squeezy (LS) for CoDRAG, including products, webhook integrations for license issuance, and configuring Netlify with the correct environment variables.

## 1. Initial Store Setup

1. Log into your [Lemon Squeezy Dashboard](https://app.lemonsqueezy.com/).
2. Navigate to **Store settings**.
3. Set the **Store name** to `CoDRAG`.
4. Ensure your **Store URL** is set (e.g., `codrag.lemonsqueezy.com`).
5. Configure your branding (logo, colors) to match the CoDRAG brand (`#A020F0` for primary purple).
...
### Product A: CoDRAG Pro (Monthly)
1. Go to **Products** → **Create Product**.
2. **Name:** CoDRAG Pro - Monthly
3. **Description:** Continuous access to the CoDRAG semantic tracing engine with monthly updates.
4. **Pricing:** Subscription, $7 / month.
5. **License Keys:** ENABLED.
6. **Activation Limit:** 2 devices.

### Product B: CoDRAG Pro (Perpetual)
1. Go to **Products** → **Create Product**.
2. **Name:** CoDRAG Pro - Perpetual (Fallback/One-time)
3. **Description:** Lifetime license to the current version of CoDRAG.
4. **Pricing:** Single payment, $79.
5. **License Keys:** ENABLED.
6. **Activation Limit:** 2 devices.

### Product C: CoDRAG Team
1. Go to **Products** → **Create Product**.
2. **Name:** CoDRAG Team
3. **Description:** CoDRAG headless runner for CI/CD and Team Sync capabilities.
...
2. Click **+ Add Webhook**.
3. **URL:** `https://payments.codrag.io/api/webhooks/lemon-squeezy`
...
4. *CoDRAG's pricing page will dynamically append these discount codes to the checkout URL based on the user's IP geolocation.*
...
NEXT_PUBLIC_LS_CHECKOUT_MONTHLY=https://codrag.lemonsqueezy.com/checkout/buy/variant_id_here
NEXT_PUBLIC_LS_CHECKOUT_PERPETUAL=https://codrag.lemonsqueezy.com/checkout/buy/variant_id_here
NEXT_PUBLIC_LS_CHECKOUT_TEAM=https://codrag.lemonsqueezy.com/checkout/buy/variant_id_here
...
2. Go to the CoDRAG pricing page (`IS_BETA_MODE` must be `false` in `websites/apps/marketing/src/app/pricing/page.tsx`).
...
5. Verify that you are redirected to `https://payments.codrag.io/success`.
6. Verify that an email containing the license key is delivered.
7. Open the CoDRAG CLI or Dashboard and attempt to activate the generated license key.
