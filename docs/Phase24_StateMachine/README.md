...
**Current pattern:** Stateless `check_feature()` / `require_feature()` functions that read a cached `License` dataclass. License is loaded once from `~/.runprep/license.json` or `PREP_TIER` env var.
