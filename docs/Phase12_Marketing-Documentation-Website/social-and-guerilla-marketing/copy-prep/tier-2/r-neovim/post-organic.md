...
3. **Avante.nvim + Prep: Using a local graph engine for context**
...
### The Tool
So I built **Prep**. It's a standalone daemon written in Rust.
*   It watches files.
*   It builds a dependency graph.
*   It exposes an MCP (Model Context Protocol) server.

### The Workflow
Now, my editor just sends a lightweight query ("Get context for symbol X") to the daemon, and Prep returns the pruned code. My Neovim UI stays snappy.
