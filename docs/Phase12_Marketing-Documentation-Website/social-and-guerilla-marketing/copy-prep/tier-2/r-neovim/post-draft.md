...
2. **CoDRAG: A local rust-based graph indexer to feed context to your AI plugins**
...
### The Solution
I built **CoDRAG**, a standalone local daemon (written in Rust) that indexes your codebase into a dependency graph. It exposes this via **MCP (Model Context Protocol)**.

### Why this fits Neovim
*   **Unix Philosophy:** Let the editor edit. Let the context engine index.
*   **Standard Protocol:** If you use an MCP client (like `avante.nvim` or others adding support), you can query CoDRAG for "all implementations of `Auth`" and get a precise, pruned context back.
