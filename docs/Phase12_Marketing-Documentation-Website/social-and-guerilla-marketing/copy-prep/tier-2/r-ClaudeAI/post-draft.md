...
3. **Better than `grep`: Using CoDRAG to feed Claude precise code context**
...
### The Solution
I built **CoDRAG**, a local MCP server designed specifically for this. It builds a graph of your code.

### Workflow
1.  Connect CoDRAG to Claude Desktop (`claude_desktop_config.json`).
2.  Ask Claude: "How is the `User` class used in `auth.py`?"
3.  Claude calls the CoDRAG tool, traces the graph, and sees *only* the relevant lines.
