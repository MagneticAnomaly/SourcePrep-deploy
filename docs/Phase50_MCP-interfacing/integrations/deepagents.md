...
> How DeepAgents consumes MCP via langchain-mcp-adapters, its agent harness architecture, and CoDRAG optimization.
...
### Key Distinction
Where Cursor/Claude Code/Gemini CLI are **end-user products**, DeepAgents is a **building block**. Developers use it to create custom coding agents. CoDRAG integration here means making CoDRAG available as an MCP tool to any LangGraph-based agent.
...
async with MultiServerMCPClient(
    ...
    # tools now contains LangChain Tool objects for each CoDRAG MCP tool
...
Without resources, prompts, or instructions, CoDRAG's **tool descriptions** are the ONLY mechanism to guide DeepAgents' behavior. The descriptions must be self-contained:

```
codrag: "Get ambient codebase context -- structural overview, module map, hub files,
...
### Sub-Agent Context
Each sub-agent has its own context window. CoDRAG's compact ambient response (~250 tokens) is ideal because:
- Sub-agents need structural context but can't afford large payloads
- The main agent can call `codrag` once and inject the result into sub-agent prompts
- Sub-agents can then call `codrag_search` for specific queries

### Context Management
DeepAgents has its own context management (summarization, window management). CoDRAG's tool responses are subject to this -- early responses may be summarized in long tasks.

---

## 5. Programmatic Integration Pattern

Since DeepAgents is code-first, CoDRAG integration is via code, not config files:

```python
# Example: DeepAgents with CoDRAG
...
async def main():
    async with MultiServerMCPClient({
        ...
        codrag_tools = mcp_client.get_tools()
        
        agent = create_agent(
            model="claude-sonnet-4-20250514",
            tools=[*built_in_tools, *codrag_tools],
            system_prompt="""You have access to CoDRAG structural code intelligence.
            ALWAYS call codrag first for module structure and hub files.""",
        )
...
Unlike IDE tools where CoDRAG generates rules files, DeepAgents' system prompt is controlled by the developer building the agent. CoDRAG's documentation should include a **recommended system prompt snippet** for LangChain/DeepAgents users.

---

## 6. CoDRAG Documentation for DeepAgents Users
...
Before making changes, use `codrag_impact` to understand dependencies.
```
...
mcp_config = {
    "codrag": {
        ...
        "transport": "stdio",
        "env": {
            "CODRAG_PROJECT": "/path/to/project"  # optional: explicit project
        }
    }
}
```
...
- [ ] Test CoDRAG MCP tools via langchain-mcp-adapters
- [ ] Verify all 5 CoDRAG tools convert correctly to LangChain Tool objects
- [ ] Test with multiple LLM providers (OpenAI, Anthropic, Google) via LangChain
- [ ] Create example integration script for DeepAgents + CoDRAG
- [ ] Document recommended system prompt snippet
- [ ] Test sub-agent context injection with CoDRAG ambient response
...
| LangChain tool schema conversion loses detail | MEDIUM | Test that all CoDRAG parameters survive conversion. |
| Developer must manually add system prompt | MEDIUM | Document clearly. Can't auto-generate for programmatic tools. |
| Sub-agent context too small for CoDRAG responses | LOW | 250-token ambient response fits any context window. |
