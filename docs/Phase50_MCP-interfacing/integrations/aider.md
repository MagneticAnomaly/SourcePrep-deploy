...
> How Aider consumes MCP, its git-native architecture, and CoDRAG optimization.
...
- Most tools (Cursor, Claude Code, etc.) are **MCP clients** that call CoDRAG's MCP server
...
Aider doesn't have native MCP client support. To use CoDRAG with Aider:
1. **Via `read:` directive**: Aider can read files into context. CoDRAG could generate a static context file that Aider reads.
2. **Via AGENTS.md**: Aider reads AGENTS.md when configured with `read: AGENTS.md`
3. **Via shell commands**: Aider can run shell commands. A `codrag context` CLI command could pipe output into Aider's context.
...
Since Aider can't call MCP tools dynamically, CoDRAG should support a **static context export**:
...
### Regeneration
The static file is regenerated after each CoDRAG pipeline run. Could be added to `.gitignore` (local only) or committed (shared with team).

---

## 4. AGENTS.md Integration

Aider reads AGENTS.md when configured. The CoDRAG section in AGENTS.md provides:
...
- [ ] Implement `codrag export` CLI command for static context file
- [ ] Document `.aider.conf.yml` setup with CoDRAG
- [ ] Test AGENTS.md reading in Aider
- [ ] Explore: can Aider run shell commands mid-session to refresh CoDRAG context?
...
| Aider users are power users who expect CLI workflows | LOW | `codrag export` fits their workflow. |
