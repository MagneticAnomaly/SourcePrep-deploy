...
3. .codrag/project.json pointer                      ← instant, no daemon needed
   Checks: IDE workspace roots → CWD → CODRAG_WORKSPACE env var
4. Auto-register .codrag/ folders not yet in daemon  ← zero-config setup
...
**Project ID source:** Explicit `project_id` param, or auto-detected from `.codrag/project.json`
...
Every CoDRAG project gets a `.codrag/project.json` pointer in its root:
