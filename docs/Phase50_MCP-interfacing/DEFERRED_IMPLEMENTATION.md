...
2. **MCP server side** (`src/codrag/mcp/server.py`):
   ...
   - `_check_atlas_signal(project_id)` -- async method called at the top of `tool_context()`:
     ...
     - Checks `<project>/.codrag/atlas_updated.signal` and `<project>/atlas_updated.signal`
...
codrag mcp --log-file ~/.codrag/mcp-test.log --debug

# 3. Verify MCP server is receiving connections
tail -f ~/.codrag/mcp-test.log
...
4. Check `~/.codrag/mcp-test.log` for `MCP client detected: name=... version=...`
