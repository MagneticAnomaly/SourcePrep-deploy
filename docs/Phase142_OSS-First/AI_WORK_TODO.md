...
- **Do:** Fix `docs_grounding.py:92,94,96,98` "Auto-generated by
  RunPrep"/"CoDRAG" markers in `PREP_SELF_OUTPUT_MARKERS` — these are detection
  patterns (not output to clients) but the codenames ship on the public mirror
  source. Decide with Eric whether to keep as detection patterns (they match
  legacy output) or rename. Fix `packages/paperclip-plugin-prep/tests/plugin.spec.ts`
  asserting `manifest.id === 'codrag'` and exercising `codrag:*` tools — update
  to `sourceprep` / `prep` per the brand split. Fix `lemon_squeezy.py:14`
  codename reference.
