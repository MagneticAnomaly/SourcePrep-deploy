...
|---|---|
| **Trademark (D5)** | ✅ **SETTLED** — Eric: "good and safe." Not a *decision* blocker; the B1/B2 filing actions remain pre-Show-HN execution tasks (file 1(b) on the Principal Register once B1's free USPTO clearance search clears). |
...
| **License (D1)** | ✅ **DECIDED (confirmed 2026-07-18): Apache-2.0 + DCO (not CLA), permanent** (no AGPL fallback). Consistent with Phase144 `RESEARCH.md` ("Decided 2026-06-15") + the marketing spec. Tradeoff accepted: DCO + permanent = no easy relicense later, in exchange for a stronger anti-rug-pull signal + lower contributor friction. A deep-research pass (prompt: `LICENSING_DEEP_RESEARCH_PROMPT.md`) is validating, not re-opening. **Two gaps still open:** (a) the cited `LICENSING_RECOMMENDATION.md` was never committed — author it as the decision-of-record; (b) root `LICENSE` is still commercial proprietary — swap to Apache after the IP assignment (C5). The site's Apache copy is unblocked; these are record/execution items. |
