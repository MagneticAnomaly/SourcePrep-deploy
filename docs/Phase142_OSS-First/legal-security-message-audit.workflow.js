export const meta = {
  ...
  description: 'Cross-surface legal + security + message-clarity audit of all public sites + repo metadata (root LICENSE is now Apache-2.0 as of commit 99315988 — verify relicense completeness); classify fix-now / flag-for-AI-deep-review / Eric-decision',
  phases: [
...
const SURFACES = [
  ...
  { id: 'repo-meta', label: 'repo metadata', paths: ['LICENSE', 'NOTICE', 'README.md', 'SECURITY.md', 'CONTRIBUTING.md', 'package.json', 'pyproject.toml', 'engine/Cargo.toml', 'websites/apps/*/package.json', 'packages/*/package.json'], note: 'The "what the project legally is" surface. Root LICENSE is now Apache-2.0 (commit 99315988); package.json/pyproject/Cargo also Apache-2.0. The metadata-vs-LICENSE contradiction is RESOLVED — instead VERIFY relicense completeness/consistency (NOTICE/CONTRIBUTING/SECURITY no longer "All Rights Reserved"? copyright-holder + year correct? DCO/CLA present? any stale proprietary header in a source file? LICENSE text correct/complete?). Flag any remaining inconsistency in the rollout, not the (resolved) contradiction.' },
]
...
const LENSES = [
  { lens: 'legal', prompt: `LEGAL lens. Root LICENSE is now Apache-2.0 (commit 99315988); package.json/pyproject/Cargo also Apache-2.0 — the metadata-vs-LICENSE contradiction is RESOLVED, do NOT re-flag it. Instead VERIFY relicense completeness: compare every extracted claim against the code-posture ground truth, root LICENSE (Apache-2.0), package.json/pyproject/Cargo, NOTICE, CONTRIBUTING, SECURITY, and across surfaces. Surface: (a) relicense-completeness gaps (any stale "All Rights Reserved" / proprietary header in a source file? NOTICE/CONTRIBUTING/SECURITY still aligned to commercial? copyright-holder + year correct? DCO/CLA present?), (b) license-header gaps, (c) trademark/copyright-holder/year inconsistencies (Magnetic Anomaly LLC vs SourcePrep), (d) DCO/CLA/CONTRIBUTING gaps, (e) privacy/terms page existence + match-code, (f) any GPL/weak-copyleft dep not disclosed, (g) NOTICE attribution completeness, (h) any surface that still treats the project as proprietary in present tense (e.g., a payments page saying "for sale" without the relicense caveat, or a docs page that could now state Apache-2.0 but is over-conservative — note: over-conservative is SAFE, do not force it to assert). For each finding give verdict/severity/classification/action. Mark GPL-vendored-source-scan as flag-deep-review (scancode not installed).` },
