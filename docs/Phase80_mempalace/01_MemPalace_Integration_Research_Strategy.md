...
> Goal: Define a rigorous methodology for evaluating MemPalace's architectural concepts (AAAK dialect, Agent Diaries, Temporal Graph) to determine where and if they should be integrated into the Prep ecosystem.

## 1. Background & Hypothesis

Our initial investigation of the [MemPalace](https://github.com/milla-jovovich/mempalace) repository revealed that while its core engine (a conversation-centric ChromaDB knowledge base) shouldn't replace Prep's codebase-anchored architecture, several of its data models and memory abstractions are highly compelling.

The hypothesis is that we can adapt specific MemPalace patterns to solve Prep's emerging challenges:
...
**Objective:** Evaluate modifying Prep's context assembly pipelines to encode variables, files, and module relationships into AAAK-like symbolic abbreviations.

**Codebase Areas to Examine:**
- `src/prep/core/compression/lod_extractor.py` and `compressor.py`: Can we inject an AAAK conversion pass?
- `src/prep/core/swarm_orchestrator.py` & `src/prep/services/pipeline/stages.py`: How does state move between workers and the coordinator? Do we compress outputs?
- `src/prep/api/routers/projects/search.py`: How are context payloads assembled, and can the prompt adapt to read AAAK dialect?
...
**Codebase Areas to Examine:**
- `src/prep/services/observation_store.py`: Examining the current `visibility` and `created_by` access patterns. Are they rigorous enough for true isolation?
- `src/prep/agents/core.py`: Reviewing `AgentCore` facade to see how `save_observation` is currently utilized by role logic.
- `src/prep/mcp/server.py`: Can we safely expose `prep_diary_read` and `prep_diary_write` MCP tools without breaking existing patterns?
...
**Codebase Areas to Examine:**
- `src/prep/services/concept_store.py`: How is the `status` (seed, active, archived) and `stale` flag managed today?
- `src/prep/core/watcher.py` & `src/prep/services/scope_orchestrator.py`: How do code changes currently invalidate stored knowledge?
