...
Experimental-essay content plan for Prep. Each essay is an experiment you actually run; the essay is the writeup. No predictions, no HN-front-page claims, no made-up taxonomies. Scenarios are scoped against Prep's *real* capabilities as audited on 2026-04-11 (see [`00_feasibility_audit.md`](./00_feasibility_audit.md)).

## What this folder contains

- [`00_feasibility_audit.md`](./00_feasibility_audit.md) — what Prep actually does today, what's half-built, what's killed. Read this first.
...
- [`02_hub_file_problem.md`](./02_hub_file_problem.md) — compare Claude Code's blast-radius answers to Prep's impact analysis on a real hub file.
- [`03_day_zero_onboarding.md`](./03_day_zero_onboarding.md) — "I just cloned this repo. What should I read first?" across three harnesses + Prep's atlas.
- [`04_aider_repomap_vs_prep_atlas.md`](./04_aider_repomap_vs_prep_atlas.md) — honest technical comparison between Aider's personalized PageRank repo map and Prep's atlas.
- [`05_where_prep_fails.md`](./05_where_prep_fails.md) — the honest dogfooding essay. Cataloged failures of Prep on a real repo. Credibility piece.
...
- [`11_code_is_a_graph.md`](./11_code_is_a_graph.md) — **flagship academic essay.** Nine-paper literature review defending Prep's core architectural bet: code retrieval should operate on a graph, not flat text. 6000-word target, 40–70 hours effort.
...
- [`25_code_is_a_graph.md`](./25_code_is_a_graph.md) — **Article D drafted** (~2100 words). Capstone piece: four years of convergent research on graph-based code retrieval. Ferrante 1987 callback. Aider and Sourcegraph cited alongside Prep.
...
- **No product pitch until the last 15%.** Earn attention with the experiment. Mention Prep as the tool that enabled the observation.
...
- Claiming Prep beats alternatives without measurement.
- Picking fights with named writers (Willison, etc.) to borrow audience.
- Scale claims Prep can't back up (e.g. "500-line implementation" when Prep is a three-language monorepo).
...
- **Killed:** "The architecture antibody" essay. Prep's concepts/antibodies framework exists as a data structure but the trigger/firing pipeline is not wired into runtime monitoring. Cannot honestly demo end-to-end today. Revisit if/when triggers fire reliably.
- **Killed:** "500 lines of Python" implementation essay. Prep is not a 500-line thing. Format collision with reality.
...
- **Added:** "Where Prep fails" — the honest failure catalog, which turns the audit's findings into content rather than hiding them.
- **Corrected:** The Aider comparison essay now knows that Aider uses personalized PageRank via `networkx.pagerank` on a tree-sitter symbol graph, while Prep's hub ranking is z-score on in-degree. Prep does not automatically win that comparison.

## Target repos (audit-approved)

Use one of Prep's existing test repos in `tests/eval/real_repos/` for the first experiments. All have been indexed before; scale risk is minimal.
