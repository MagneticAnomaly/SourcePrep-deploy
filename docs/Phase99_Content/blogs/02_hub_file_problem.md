...
**Depends on:** `prep_impact` MCP tool (transitive, max_hops configurable), hub detection via `prep_audit`, a Claude Code session without Prep attached, and a Claude Code session *with* Prep attached.
...
2. **Find the hub.** Run `prep_audit` and extract the top hub file (highest in-degree, or highest z-score on the hub bottleneck check). For `gin-go` this is likely `context.go`; for `click-python` it's likely `core.py`. Verify by hand — if Prep's hub detection picks something trivially utility-ish (constants, logging helpers), pick the second or third result instead.
...
4. **Trial A — Claude Code alone.** Start a fresh Claude Code session with *no* Prep MCP server attached. Open the repo. Ask: *"I'm considering renaming `<function_name>` in `<file>` to `<new_name>`. What do I need to check before making this change?"* Let it work. Log every file it reads (Claude Code shows its tool calls). Save its full answer.
5. **Trial B — Claude Code + Prep.** Fresh session. Same repo. Prep MCP server attached. Same question. Log tool calls and the answer.
6. **Ground truth.** Run `prep_impact` directly on the function with `max_hops=3` or higher. Also run a simple grep for the function name as a sanity check. These two together are your ground truth for "what depends on this function."

## What to record

- Raw `prep_audit` output showing hub detection
- The chosen function and its signature
- Full transcript of Trial A (files read, answer given)
- Full transcript of Trial B (files read, `prep_impact` output if called, answer given)
- The ground-truth dependent list from direct `prep_impact` + grep
...
The argument is not "Claude Code is bad." It's more precise: **without structural priors, a tool's blast-radius estimate is a guess wearing a confident voice.** Claude Code may read several files and form a reasonable answer, but it has no systematic way to know when it's done. Prep's contribution is not intelligence — it's *completeness*. A graph-backed impact query is either correct or wrong; it doesn't trail off.
...
- **Prep might get it wrong.** If `prep_impact` misses a caller that grep finds, that's an honest limitation to report. It probably means the parser missed a dynamic import or a reflection-based call. This is information and belongs in the essay.
- **The comparison might be confounded.** Claude Code with web access or advanced tool use might read the same files Prep identified by a different route. If so, the essay shifts: "Claude Code arrived at the same answer by reading 14 files; Prep arrived at it by running one query. The cost difference is the story, not the correctness difference."
...
- `prep_impact` traverses the static import/call graph. It cannot see dynamic dispatch, reflection, late binding, plugin systems, or anything that hides edges at runtime. A plugin-heavy codebase would weaken the argument.
- Prep's hub detection uses z-score on in-degree (see `00_feasibility_audit.md`). This is simpler than Aider's personalized PageRank. For some repos, Prep might pick a hub that PageRank would deprioritize. Note this up front.
...
- Prep's hub detection source (`src/prep/core/audit/analyzers/hub_bottlenecks.py`)
- Aider's repo map (for readers who want to see a different serious attempt)

## Next action

Run this only after essay 01 has confirmed the experimental format works. Needs Claude Code + Prep wired up together, which adds one more failure mode to manage.
