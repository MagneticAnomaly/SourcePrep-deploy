...
| `PROPOSAL_playwright-uat-harness-v1.md` | proposal — **T1 + T2 shipped 2026-06-22, T3–T5 awaiting execution** | partial | Methodology + 5-task plan to extend the existing `tools/playwright_smoke.py` with 4 Phase 145 §8 invariants (I1, I2, I3, I13), drive HomeColab through 4 operations × 3 iterations, produce `SCORECARD_uat_*.md` baselines. Independent of all fix proposals. **T1 (invariant library) + T2 (wired into `watch_until_idle` with 2-tick persistence gate) shipped 2026-06-22. Four-lens adversarial review run pre-merge each drop; Tier 1+2 fixes landed in-drop, Tier 3 gaps tracked in PROPOSAL §9.1 + §9.3 follow-ups.** |
| `tools/phase145_uat/{constants,invariants}.py` + `tests/test_phase145_invariants.py` + `tools/playwright_smoke.py` + `packages/ui/.../GraphEnrichmentPipeline.tsx` | shipped code — T1 + T2 of UAT harness | live | **T1:** I1/I2/I3/I13 invariants with `ACTIVE_PIPELINE_PHASES = {running, pausing, paused, recovering, cancelling}`. **T2:** invariants wired into `watch_until_idle` per API-poll tick; `INVARIANT_PERSISTENCE_TICKS=2` absorbs SSE-commit-lag races; `data-testid="current-stage-indicator"` + `data-testid="last-run-chip"` land on `GraphEnrichmentPipeline.tsx`; new `invariant_failure` event kind + `invariant_failure_count` + `Invariants` column in `report.md`. **50 pytest cases** (43 invariant + 3 collapsed-group + 4 persistence-gate). Selector contract + cross-group I3 gap documented in module docstring + PROPOSAL §3/§9.1. |
