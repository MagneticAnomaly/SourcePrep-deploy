...
| `SCORECARD_uat_revalidation_2026-07-22.md` + `.manifest.json` sidecar | **revalidation anchor** | live | First harness run since 2026-06-23 (single Op-2 iter, PMR). Harness plumbing fully working post-PR-A..PR-O; **3 real desyncs in one 65s incremental run** — `group_reasoning` stale spinner (§2r shape), `clustering` api_running_dom_not_running (§2r shape), `atlas` dom_claims_running_while_api_idle at 67% (96D/§2b shape, cross-group — §9.3 #18 adjacent). All 4 shipped invariants held: these live in desync-subtype territory the invariants deliberately don't cover. Run with the daemon NOT restarted (Applivation-Android holds a deliberately-paused ~9.7-day-old deep_enrichment run). |
| `FINDING_force-from-start-rebuild-hangs-pre-registration-orphans-all-barrier.md` | finding | §2v — partially mitigated (operational recovery §12 + harness defenses); code fixes await sign-off | Rebuild hangs pre-registration (step 3.5), orphaning an `all`-scope barrier that survives restart and silently no-ops every later rebuild. §11: `/rebuild/stop` hangs too; wedge accumulates with daemon uptime (~8h); fresh daemon clean. §12 recovery sequence; §13 validates clear-on-success is sound. Blocked ALL pipeline runs on the dogfood daemon 2026-07-12→22. |
| `PROPOSAL_rebuild-pre-registration-hang-and-barrier-safety-v1.md` | proposal — **draft, awaiting sign-off** | open | A1 barrier acquire-after-registration + failure-side clear (highest leverage) → A2 bounded launch/stop timeouts → A3 daemon stderr capture + faulthandler → A4 un-swallow launch exceptions → A5 early journal registration → A6 during-lifetime watchdog; A7 `_lock`-across-I/O hygiene deferred to v2. Per-area tests + dogfood validation plan. |
...
| `.claude/skills/uat-orchestrator/SKILL.md` | shipped skill — **T5 of the UAT harness** | live | The hours-long campaign orchestration layer for any future session: real-repo roster with measured durations + measure-first rule, safety rules (daemon-restart gated on paused queue entries; never mutate target repos; off-limits list), the session→triage→file→decide loop, stop conditions (LLM exhaustion, §2m stall, §8 red flags), corpus filing conventions. Companion prereqs shipped same day: §9.3 #5 capture logs, #6 `--retry-non-pass`, raisable watch budgets for big repos. Authored TDD-style (RED baseline → GREEN → adversarial verify) per superpowers:writing-skills. |
...
| `PROPOSAL_auto-incremental-three-gates-v1.md` | proposal — **draft, awaiting scrutiny** | open | §2q/§2s (auto-refuses half) — root cause pinned as three compounding gates: (RC#1) "close enough" coverage gate suppresses source files at high coverage; (RC#2) `trigger_build` always returns `True` so the watcher silently drops pending paths when `run_fast_sync` returns `False`; (RC#3) downstream-partial guard permanently blocks incremental fast_sync when §2n (ANTIBODIES never completes) keeps `finalize_resume=4/5`. Four fix areas A1–A4 + risk register R1–R10 + 8 OQs. **OQ1 is the gate: does ANTIBODIES write output files on zero derivables? If yes, A3 option (a) doesn't fully fix RC#3.** |
...
### 2q. Auto-incremental pipeline never auto-triggered despite hours of untracked files (open, 2026-06-18; root cause pinned 2026-08-20)
...
- **Root cause pinned (2026-08-20): three compounding gates.** (RC#1) The coverage-check backstop's "close enough" gate (`watcher.py:596-616`) suppresses the trigger when `stale==0 and coverage_pct>=95 and untraced<=20` — regardless of whether the untraced files are source or binary. 9 untraced `.py`/`.md` files at 99.1% coverage → suppressed forever. (RC#2) `trigger_build` (`watch.py:160-166`) always returns `True` even when `run_fast_sync` returns `False`, so the watcher silently clears its pending set and transitions to idle — the untraced files are dropped. (RC#3) `run_fast_sync`'s downstream-partial guard (`orchestrator.py:771-822`) blocks incremental fast_sync when `0 < finalize_resume < 5`; with §2n (ANTIBODIES never completes), `finalize_resume=4` permanently → all incremental triggers return `False`. Only `force_from_start=True` (manual Rebuild) bypasses RC#3. The three form a vicious cycle: §2n → RC#3 → RC#2 → RC#1 → files pile up → manual Rebuild → cycle restarts.
- Hypotheses H-Q3 (debounce gated → actually `run_fast_sync` gated, RC#3) and H-Q6 (`_check_incomplete_deep_enrichment` stuck on §2n → actually the downstream-partial guard in `run_fast_sync` stuck on §2n, RC#3) are **confirmed**. H-Q1 (watcher not started), H-Q2 (FSEvents missed), H-Q5 (glob filter) are **ruled out**.
- **Proposal:** [`PROPOSAL_auto-incremental-three-gates-v1.md`](PROPOSAL_auto-incremental-three-gates-v1.md) — four fix areas A1–A4, awaiting scrutiny. **OQ1 is the gate: does ANTIBODIES write output files on zero derivables? If yes, A3 option (a) doesn't fully fix RC#3.**
...
- **2026-06-18 23:38:** the daemon-freeze attribution from 22:38 is **withdrawn** — §2q stands as a standalone watcher-side bug. New adjacent symptom §2s (Edge Discovery stuck `Pending` with auto-incremental refusing to fire) is the same underlying bug surfacing on a different surface (auto-refuses half; the spinner-vs-Pending half is the state-machine re-centering family).
...
### 2v. `force_from_start` rebuild hangs pre-registration; orphaned `all`-barrier survives restart and silently blocks all rebuilds (partially mitigated, 2026-07-22)

- User clicks Rebuild; "last built" never advances; no error anywhere. Under the hood: the launch hangs at orchestrator step 3.5 (`watcher.clear_pending_state` / `check_coverage_gap`) **before the run is registered** — no journal row, no crashed-run entry, and the `all`-scope `.reset_barrier` written at step 0 is orphaned. The subsumption check then silently no-ops every subsequent rebuild. The barrier is a FILE, so **daemon restart does not clear it** — the months-old "restart fixes everything" pattern broke here.
- Corollaries (finding §11): `/rebuild/stop` hangs the same way; `rm` of the barrier file is unreliable on a wedged daemon (rewritten by a background path); the wedge accumulates with daemon uptime (~8h observed) and does NOT reproduce on a fresh daemon; barrier clear-on-success is sound (§13 validation — full 15-stage rebuild completed 2026-07-22 23:02, barrier auto-cleared).
- **Mitigated operationally** (finding §12 recovery: `rm .reset_barrier` then restart, in that order) and **defended in the UAT harness 2026-07-23**: `run_session` aborts rc=4 on an orphaned barrier (never self-clears — continuing farms false results) and forced builds that produce zero activity log `no_activity_after_forced_build` as an error (pre-fix the `no_activity` branch scored the silent block as a PASS). **Code fixes await proposal sign-off.**
- **Full root cause + evidence:** [`FINDING_force-from-start-rebuild-hangs-pre-registration-orphans-all-barrier.md`](FINDING_force-from-start-rebuild-hangs-pre-registration-orphans-all-barrier.md). **Fix plan:** [`PROPOSAL_rebuild-pre-registration-hang-and-barrier-safety-v1.md`](PROPOSAL_rebuild-pre-registration-hang-and-barrier-safety-v1.md) (A1 barrier safety → A2 launch timeouts → A3 stderr capture → A4 un-swallow → A5 early registration → A6 lifetime watchdog; A7 lock-hygiene follow-up).
