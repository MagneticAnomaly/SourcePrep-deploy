...
> **STATUS: v1 COMPLETE — T1 + T2 + T3 + T4 SHIPPED 2026-06-22, T5 (cadence/orchestration) SHIPPED 2026-07-22 as the `uat-orchestrator` skill.** Authored so that Opus (or Fable later) can pick up and execute after a computer restart without re-deriving context. This is a **methodology + extension** proposal — the underlying Playwright tooling already exists (`tools/playwright_smoke.py`). What's new is the invariant assertion layer, the operation matrix, the iteration loop, and the scorecard format.
>
> **T4 status (2026-06-22):** baseline SCORECARD captured at `SCORECARD_uat_baseline_2026-06-22.md` (+ `.manifest.json` sidecar). Subject was switched from HomeColab to PowerMateReborn after the first dry run measured 45-90 min per HomeColab rebuild — 12 iters would have been 9-18h, well over the §4 3-4h budget. PMR is 15 src files; 12 iters took ~2.3h. The dry run also surfaced a Tier-1 cancel-quiesce empty-body false negative in T3's `run_session.py` (cancel post had no body → silent 400 → daemon orphan would have poisoned every subsequent iter) — closed in §9.1 and the same commit added 3 pytest cases (50 total). T4 baseline surfaces three real bug classes: I3 intra-group stale-leak (11/12, §2r dominant), I1 double-running (4/12, §2r sporadic), I13 per-row spinner missing (5/12, §2u §6.2 sporadic despite T2 fix).
>
> **Post-I3-fix status (2026-06-22):** `SCORECARD_uat_post-i3-fix_2026-06-22.md` captures the post-fix baseline. Same 4 ops × 3 iters × PMR. **All 20 invariant fires from baseline closed: I3 11→0, I1 4→0, I13 5→0.** I3 fix shipped as `i3SafeStageState` + `shouldApplyFreezeGreen` helpers in `packages/ui/src/components/trace/freezeGreen.ts` + threading of `fastCurrentStage`/`deepCurrentStage` from `EnrichmentState` → `useEnrichment` → `App.tsx` → `useDashboardPanels` → `GraphEnrichmentPipeline`. 27 pytest cases pin the helper contract. I1 and I13 closing was an emergent win — they were downstream symptoms of the same per-stage-state-leak root cause, not independent bugs. 8/12 bare-fail rows (`status=FAIL` with all invariants ✓ and `summary.json pass=true`) persist as the §9.3 #17 anomaly — NOT a regression.
>
...
**Deferred to T2 follow-up** (see §9.3): shared scrape per tick, StageRow vitest pinning, smarter per-invariant dedup key. (`API_PHASE_TO_CANON` dead-code cleanup landed in the §9.3 #17 second-pass audit — deleted, see #17 entry below.)
...
### T5 — Wire the harness into the agent's regular check-in pattern  *[SHIPPED 2026-07-22]*

**Shipped as the `uat-orchestrator` project skill** (`.claude/skills/uat-orchestrator/SKILL.md`) rather than a section in this doc — a skill is loadable by any future session on trigger phrases ("run UAT for a few hours", "overnight campaign"), which is the actual check-in mechanism an agent has. The skill encodes: the campaign loop (session → triage → file → decide), the real-repo roster with measured durations + a measure-first rule for unmeasured repos, the safety rules (daemon-restart gate on paused queue entries, never mutate target repos, off-limits targets), stop conditions (LLM exhaustion, §2m stall signatures, §8 red flags), and the corpus filing conventions (SCORECARD naming, DIFF blocks, FINDING_/§2x/index rows). Authored TDD-style per superpowers:writing-skills: RED baseline (fresh agent asked to plan an hours-long campaign from repo docs alone — surfaced 6 gaps incl. a dangerous `git stash` improvisation from this doc's RU2 and guessed budgets), GREEN (skill written against those gaps), then an adversarial verify pass (plan-with-skill judge + fact-checker + safety reviewer). §6 below remains the cadence rationale; the skill is its operational form. Prereqs shipped the same day: §9.3 #5 (capture logs), #6 (--retry-non-pass), and the raisable watch budget (single-knob big-repo support).
...
| **`cancel_and_quiesce` empty-body false negative** *(addressed in T4 dry run 2026-06-22)* | `run_session.cancel_and_quiesce` posted `httpx.post(url)` with no body. `/pipeline/cancel` requires `{group, reason}` per `CancelRequest`; an empty post returns 400 silently because `httpx` does NOT raise on non-2xx. Result: the cancel never landed, `wait_for_pipeline_idle` timed out at 30s, the daemon kept running stages 6-15, and every subsequent iter would have 409'd on `/pipeline/rebuild`. T3's lone `TestCancelAndQuiesce` test pointed at port 1 (no real endpoint) so the body shape was never asserted. Detected live during the first T4 baseline attempt against HomeColab. | Cascade-fail across all ops — one subprocess timeout poisons every subsequent iter, exactly the failure T3 added cancel-and-quiesce to prevent. | **Closed in T4 dry run**: `cancel_and_quiesce` now (1) snapshots `/pipeline/status`, (2) posts cancel for every group whose phase is in `_CANCELLABLE_PHASES = {running, pausing, paused, recovering, cancelling, queued}` with `json={"group": <g>, "reason": "uat_session_cleanup"}`, (3) treats 409 NOT_RUNNING as "already-stopped", (4) surfaces status codes in the notes string. Added `_pipeline_snapshot` helper. Three new pytest cases pin: (a) the exact body shape via monkeypatched `httpx.post` capture, (b) 409 NOT_RUNNING is not a cancel-error, (c) multiple simultaneously-active groups all get cancelled. |
| **I3 intra-group stale-leak (dominant §2r bug)** *(closed post-T4 2026-06-22)* | When a rebuild starts, dashboard per-stage rows for stages-not-yet-run in this rebuild kept rendering `data-stage-state="complete"` from the prior build. Two distinct code paths: (a) the `wasCompleteRef`-driven freeze-green override at the bottom of `GraphEnrichmentPipeline.tsx`'s state computation block, applied unconditionally to any previously-complete stage that regressed to a transient state; (b) the per-stage `compute*State` / `finStageState` functions returning `'complete'` directly whenever underlying data existed on disk, regardless of whether the stage had been re-run in the current rebuild. Baseline T4 fired I3 in 11/12 iters; bonus, I1 and I13 fired alongside (4/12 and 5/12 respectively) because the same row-state leak created the multi-running-row and missing-spinner-on-running-row race windows downstream. | §2r (intra-group), via Op-1/Op-2/Op-3/Op-4. Cross-group leak remains a separate known gap (see entry above). | **Closed post-T4**: `packages/ui/src/components/trace/freezeGreen.ts` ships two pure helpers — `shouldApplyFreezeGreen` gates the rebuild override on stage position vs group's current_stage, `i3SafeStageState` wraps each `compute*State` output and overrides downstream `'complete'`/`'stale'`/`'warning'` to `'not_built'`. Helpers consume new `fastCurrentStage`/`deepCurrentStage` props threaded from `EnrichmentState` (new fields + `SYNC_RUNNING` payload) → `useEnrichment` (3 dispatch sites) → `App.tsx` → `useDashboardPanels` (interface + wiring) → `GraphEnrichmentPipeline.tsx` (props + 10 stage-construction wrap sites). 27 pytest cases pin: per-group position lookup, augment→catalogue alias normalization, fail-open paths when group/stage unknown, real T4-fire reproduction scenarios. Post-fix SCORECARD `SCORECARD_uat_post-i3-fix_2026-06-22.md` records 0/12 fires for all three invariants. I1 and I13 closing was emergent — both were downstream symptoms, not independent bugs. |
...
5. **Stream subprocess output to file** *(Lens A #2)* — **SHIPPED 2026-07-22 (T5 prep commit).** `run_one_iter` gains `capture_path`; `main()` always passes `<out_root>/capture/<session>_<op>_iter<N>.log`. Child stdout streams there (RAM-flat AND tail-able by an unattended operator or an orchestrating agent); stderr stays PIPE'd for the bounded tail. Handle closed on all paths including TimeoutExpired (a leaked fd per timed-out iter would accumulate over an hours-long session). Deviation from the draft: capture lives under `out_root/capture/` keyed by session+op+iter, not `<run_dir>/runner_capture.log` — run_dir doesn't exist until the child creates it, and a crashed child would leave the capture orphaned; no `tee` to parent stdout (the parent prints the capture path instead — `tail -f` is the operator contract). 4 pytest cases (`TestCaptureToFile`).
6. **`--retry-non-pass` flag for resume** *(Lens A #5)* — **SHIPPED 2026-07-22 (T5 prep commit).** As drafted: with `--resume`, drops any recorded `status != 'pass'` IterResult from the loaded manifest so those iters re-run; without the flag, resume semantics are unchanged. Without `--resume` the flag is a loud rc=2 misconfiguration error (WARN→fatal philosophy, Lens C #6). The drop is in-memory only — an aborted resume (daemon down at the initial probe, rc=3) leaves the on-disk manifest untouched so history can't be destroyed with no work done in exchange. 4 pytest cases (`TestRetryNonPass`).

    Shipped alongside (same commit, big-repo enabler): **watch budgets became raisable.** New `_effective_watch_budget(requested, raisable)` in `playwright_smoke.py` — for heavy watches (initial/incremental/rebuild families) `--max-watch-secs` now REPLACES the built-in 60m/30m budget (raise or clamp) instead of clamp-only; scoped-reset watches pass `budget_raisable=False` and keep clamp-only semantics. Single-knob contract: `run_session --per-iter-timeout N` → child watch budget `N-30`. Pre-fix, a HomeColab-class rebuild (45–90 min) timed out at the hardcoded 60m ceiling regardless of `--per-iter-timeout`, which silently blocked UAT on any real repo bigger than PMR. 6 pytest cases (`TestWatchBudgetRaise`); the two `_capped_max_seconds` pins are unchanged (helper retained for the non-raisable path).
...
17. **Bare-fail-no-invariant anomaly** *(reframed + fix shipped 2026-06-23 as `079a262f`, live-smoke-validated)* — Originally characterised as "rc-vs-summary mismatch with `pass=true`"; a direct walk of every bare-fail iter's `summary.json` + `events.jsonl` showed the original diagnosis was wrong — `summary.pass` is actually **false** in every case. The 8 bare-fail rows split into three distinct real-failure classes that all rendered as `✓✓✓✓ + FAIL` because `parse_iter_result` only walked `kind="invariant_failure"` events:
    ...
    - **Op-3/Op-4 (6 iters):** 1 `error` event each, detail `"timed out"`. httpx 30s ReadTimeout on `api.rebuild()` — the daemon was holding the request through post-iter cleanup work that exceeded the client's default timeout. Not the new `--refresh-at-secs`/`--update-at-secs` codepaths themselves — the cascade from Op-2 iter 3's incomplete settle.

    Fix shipped (tools/ only, no daemon changes):
    - **Fix A** *(reporting gap)* — `parse_iter_result` now also walks `kind in {error, desync}` and surfaces a structured summary (e.g. `"error×1 'timed out'"`, `"desync×3 'api_complete_dom_still_running'"`) in `IterResult.notes` whenever `status=fail AND failures=[]`. Truncated to 180 chars so the markdown table stays readable. Suppressed when the smoke flagged pass OR a shipped invariant fired (failures-list is authoritative).
    - **Fix B** *(cascade root)* — `wait_for_pipeline_idle` previously checked only `phase == "running"`; a daemon in `queued`/`pausing`/`recovering`/`cancelling` reported as idle. Now checks `phase NOT IN {idle, completed, cancelled, failed}` (the four terminal phases per `state_machine.py:94-200`). Closes the Op-2-iter-3 → Op-3-iter-1 cascade window.
    - **Fix C** *(resilience)* — `Api.rebuild()` and `Api.run_all()` use a per-call 120s httpx timeout (vs the 30s class default) since these endpoints can spend tens of seconds on daemon-side reset/hydrate work before returning. Status/cancel calls keep the short default.

    Pre-fix coverage: 13 new pytest cases (5 for Fix A, 6 for Fix B, 2 for Fix C) all RED before implementation, GREEN after. Live re-parse of the post-I3-fix run dirs through the new code shows every bare-fail row now carries actionable signal in its notes string.

    Pre-push adversarial scrutiny (4 investigators + synth + 2 verifiers, both verifiers CONFIRMED) found 5 critical post-merge gaps + 8 soft follow-ups, addressed in the same PR:
      - **#1 cancel-quiesce annotation bypassed the 180-char cap** — `main()`'s post-`parse_iter_result` append happens after the cap, so a long cancel-quiesce detail could push notes past SCORECARD cell width. New `_clip_notes()` helper + bumped cap to 220 chars + wrapped the append.
      - **#2 cancel-quiesce default 30s too tight for `recovering` daemons** — main loop now passes `max_seconds=90` explicitly (recovering daemons need 60-90s normal post-crash hydrate).
      - **#3 only `events[0]` detail rendered** — mixed-cause clusters (e.g. timeout + 409 + ConnectionRefused) collapsed to one. New `_summarise_event_kind()` renders up to 3 distinct values with `(+N more)` suffix.
      - **#4 heavy-timeout decorator covered only 2 of 7 heavy endpoints** — `destroy / run_fast / run_deep / enrichment_full_reset / finalize_full_reset` all now use the 120s per-call timeout. `--modes initial` and scoped-rebuild paths no longer reproduce the same cascade.
      - **#5 `playwright_smoke.is_any_running` checked only `phase=="running"`** — same root-cause class as Fix B but in the smoke's watch loop; smoke would declare "settled" mid-transition. Now uses the same non-terminal predicate as `wait_for_pipeline_idle`.

    SOT enforcement landed alongside: new `TestHarnessPhaseSetsMatchCanonical` asserts `_TERMINAL_PHASES ∪ _CANCELLABLE_PHASES` is a true partition of canonical `PipelineState` at `src/prep/services/pipeline/state_machine.py:94-104`. If the state machine adds, renames, or removes a phase, the test fails loudly — preventing the silent drift that motivated `PROPOSAL_state-machine-re-centering-v1.md`. The harness's `_TERMINAL_PHASES` is an INTENTIONAL superset of canonical `TERMINAL_STATES` (adds `idle`, since `idle` daemons accept new work just like terminal ones) — pinned by `test_canonical_terminal_states_are_subset_of_harness_terminal`. Comment in `run_session.py` corrected from the pre-fix "four terminal states" misnomer.

    Total coverage after scrutiny: 98 pytest cases (50 pre-existing + 18 from initial #17 fix + 30 from scrutiny round). Scrutiny round identified the harness's three remaining drift sites — `is_any_running` (closed in #5), `api_stage_verdict`, `API_PHASE_TO_CANON` — the latter two are soft follow-ups since they don't break the cadence-readiness criterion.

    **Second-pass structural audit (2026-06-23, 5 investigators + synth + 2 verifiers, verdict needs_revision → revised, then confirmed):** The first scrutiny pass deferred `api_stage_verdict` + `API_PHASE_TO_CANON` + `_check_group_phase_consistency` as soft follow-ups. The second pass treated them as real drift sites and addressed them as part of a "phase-vocabulary unification" PR scoped narrowly per verifier 2's split recommendation (PR-A vocabulary now; PR-B subprocess/timing hardening deferred). Verifier 1 caught two errors in the synthesis I would have shipped otherwise: (a) `api_stage_verdict` extension to ALL `ACTIVE_PIPELINE_PHASES` would produce false positives because `pausing`/`cancelling`/`recovering` have a TRANSITIONAL `current_stage`; narrowed to `_VERDICT_ELIGIBLE_PHASES = {running, paused}` (stable snapshot only); (b) the first scrutiny's SOT scorecard claimed `ACTIVE_PIPELINE_PHASES` and `_NON_RUNNING_PHASES` were pinned but no tests existed — both pins now landed.

    Second-pass fixes shipped (tools/ only):
      - `api_stage_verdict` (`playwright_smoke.py`) — now allows `paused` (per stable `current_stage`); `pausing`/`cancelling`/`recovering` still excluded.
      - `_check_disk_consistency` — same gate change for the same reason.
      - `_check_group_phase_consistency` — fires on canonical `TERMINAL_STATES` ({completed, failed, cancelled}) not just `completed`. Catches spinner-after-failure / spinner-after-cancel UI lies.
      - `API_PHASE_TO_CANON` — deleted (zero call sites, stale 7-entry dict vs canonical 10).
      - Hard-coded `("fast_sync", "deep_enrichment", "finalize")` tuple at 4 sites — now imports `invariants.GROUPS` (single source).
      - 3 new SOT pin test classes covering `ACTIVE_PIPELINE_PHASES`, `_NON_RUNNING_PHASES`, `_VERDICT_ELIGIBLE_PHASES`. Plus a `TestStageOrderPinnedToCanonical` pair that parses `freezeGreen.ts` arrays via regex and asserts equality with canonical `src/prep/services/pipeline/stages.py StageId` enum lists — catches drift across the three-language stage-name mirror.

    Total coverage after second pass: 109 pytest cases (98 → 109, +11). PR-B (subprocess + timing hardening — start_new_session, monotonic clock, child-watch-budget reconciliation, httpx teardown catches, parse_iter_result empty-events tests) DEFERRED per verifier 2's "split into 2 PRs that ship vs 1 PR that stalls in review" recommendation.

    Status note: §9.3 #19 (I1/I13 closure re-validation) is now reachable because clean SCORECARDs will be readable.
18. **Cross-group stale-leak (§9.1 known gap)** *(unchanged by I3 fix)* — `i3SafeStageState` only knows its own group's current_stage. If `deep_enrichment` is running while `finalize`-group rows still show `complete` from a prior run AND the finalize group's API phase is in `{idle, queued, completed}`, no anchor exists and `i3SafeStageState` returns the row state unchanged. Closing this needs the per-row "this-run vs prior-run" provenance attribute called out in the §9.1 cross-group-stale-leak entry. Separate work item; T4 baseline did NOT exercise this case (PMR is small enough that finalize completes before deep_enrichment re-runs).
19. **Why I1 and I13 closed alongside I3** *(post-fix observation)* — The workflow's per-bug agents had hypothesized I1 (useEnrichment.ts OR-precedence) and I13 (missing fast/deep force-running guard) as independent code paths. Live data refuted this: zero I1 and I13 fires after the I3 fix landed, despite useEnrichment.ts being untouched. The leak in per-stage state computation was creating the multi-running-row race window I1 detected, and the spinner-on-wrong-row race window I13 detected. Document the I1/I13 hypotheses as falsified in REFERENCE_canonical-pipeline-behavior.md once it gets a working note for invariant-vs-root-cause mappings — and re-validate over more iterations before declaring permanent closure (4/12 and 5/12 baseline rates leave room for sporadic-flake explanation).

**From post-#17-fix smoke (2026-06-23):**

20. **Daemon-side: `POST /pipeline/rebuild` holds >120s on PMR + `.reset_barrier` does not auto-clear after timeout** *(newly surfaced now that #17 fix exposes "timed out" in the Notes column)* — Op-1 and Op-3 of the post-#17-fix SCORECARD timed out the httpx client at exactly the 120s `_HEAVY_POST_TIMEOUT_S` budget; immediately after the run, `curl /pipeline/status` returns `barrier.active=true, age_seconds=247, reason='rebuild'` with **no group running**. PMR is a 15-file project — `/pipeline/rebuild` should not hold the request that long for a small repo, and the barrier should auto-clear when the reset work completes. Two distinct daemon-side bugs: (a) rebuild handler does synchronous heavy work instead of returning 202 Accepted + spawning the work async; (b) barrier lifecycle does not handle the "client cancelled mid-reset" case. Maps to `FINDING_reset-barrier-stuck-on-failed-finalize.md` §2l (Thread B). Separate PR — Phase 145 harness fix cannot resolve this from tools/. **The fact that the SCORECARD now points at this bug is the value the #17 fix delivers.**

21. **Op-4 subprocess crash during Playwright context-manager teardown** *(pre-existing harness flakiness, newly visible)* — Op-4 (`--update-at-secs 20`) of the post-#17-fix SCORECARD produced `status=ERR` with a Python traceback tail showing `contextlib.py:158 self.gen.throw`. Subprocess crashed before creating its `run_<ts>/` output directory — `run_one_iter` returned `(None, rc=1, stderr_tail)` and the iter recorded as ERR. Likely a Playwright browser/context teardown race when the page reload + update fire close together. Surface area: `playwright_smoke.run_rebuild` or the surrounding `with browser.new_context() as ctx` in `main`. Mid-priority follow-up; doesn't block T5 cadence (cadence runs would record ERR but the cause is one-off, not a recurring detector signal).

22. **PR-B: harness subprocess + timing hardening — SHIPPED 2026-06-22** *(originally deferred from second-pass scrutiny per verifier 2's split recommendation)* — A bundle of 7 items that all touch process lifecycle / clock behavior. Each item independently verified by the workflow's adversarial completeness audit. Landed as one bounded PR titled "Phase 145 harness process + timing hardening" (~370 LOC including tests). B8 (cancel snapshot→post race) intentionally deferred — narrow window, current 90s budget absorbs it; loop-until-empty fix is a future hardening pass.
    - **B1 — subprocess lifecycle.** `tools/phase145_uat/run_session.py` `run_one_iter` rewritten from `subprocess.run` to `subprocess.Popen(start_new_session=True)` + explicit `_kill_pgid_safely(pgid)` on `TimeoutExpired`. New module-level `_PENDING_CHILD_PGIDS` + `atexit.register(_atexit_kill_pending_children)` covers the OOM / segfault path. CLOSES: chromium grandchild orphan on parent SIGKILL.
    - **B2 — child watch budget reconciliation.** New `--max-watch-secs` CLI flag in `playwright_smoke.py` installs a module-level `_MAX_WATCH_SECS_CAP`; `watch_until_idle` clamps `max_seconds` at the top via `_capped_max_seconds(...)`. `run_session.py` passes `max(60, per_iter_timeout - 30)` (60 s floor protects against operator-misconfigured tiny timeouts). Child can now reach `ctx.snap(page, "timeout")` + flush stderr before parent SIGKILL — DOM evidence preserved for §9.3 #20 triage.
    - **B3 — monotonic clock.** `watch_until_idle`'s `t0` → `t0_mono = time.monotonic()`; loop computes both `now_mono` (elapsed math, next_poll, idle_since) and `now_wall = time.time()` (Event timestamps only). Laptop sleep mid-iter no longer fires false timeouts. Post-reload-recovery log line refetches both clocks.
    - **B4 — Op-4 httpx teardown.** scheduled_update except block adds `isinstance(e, (httpx.TimeoutException, httpx.HTTPError))` branch that downgrades to a `note` event (`scheduled_update_transport_error`) instead of error_count++. `main()` body wrapped in try/finally that closes `api.http`. CLOSES §9.3 #21 (`contextlib.py:158 self.gen.throw` traceback class).
    - **B5 — cancel_and_quiesce partial-failure tests.** New `TestCancelQuiescePartialFailure` (2 cases) pins per-group outcome surfacing under mixed 200/500 + unexpected-2xx-body shapes.
    - **B6 — parse_iter_result empty-events test.** New `TestParseIterResultEmptyEvents` (3 cases) pins the §9.3 #17 bare-fail empty-notes row class so refactors can't silently start fabricating notes from missing data.
    - **B7 — _summarise_event_kind ↔ _md_cell integration.** New `TestSummariseEventKindMdCellIntegration` (3 cases) pins that pipe / newline metacharacters in event detail text survive the full pipeline (parse_iter_result-style assembly → clip → md_cell) without splitting the SCORECARD table row.
    - **B1-B4 pin tests added alongside:** `TestSubprocessLifecycle` (5), `TestChildWatchBudgetClamp` (5), `TestMonotonicClock` (2), `TestOp4HttpxTeardown` (2).

    Total coverage after PR-B: **131 pytest cases in `tests/test_phase145_run_session.py`** (109 → 131, +22). All 181 phase-145 tests green (`test_phase145_invariants.py` + `test_phase145_run_session.py`). No new phase enumerations introduced — SOT discipline preserved.

    **Deferred to follow-ups:** B8 (cancel snapshot→post idle→active race in `_pipeline_snapshot` → `cancel_and_quiesce`). Narrow window, current 90 s budget absorbs it. Loop-until-empty fix is a future hardening pass, not a fire.

    **Adversarial scrutiny pass (3 investigators + synth + 2 verifiers, verdict ready-to-merge):** Both verifiers confirmed synthesis. Two T2 lexical-pin gaps surfaced (`TestMonotonicClock` and `TestOp4HttpxTeardown` were entirely `inspect.getsource` substring matching) — addressed in the same PR by adding 4 structural-pin tests:
      - `test_idle_since_and_next_poll_are_monotonic` — pins `now_mono >= next_poll` and `idle_since = now_mono` at the call site, not just `time.monotonic` symbol existence.
      - `test_event_timestamps_in_watch_loop_use_now_wall` — fails on any stray `Event(now, ...)` (must be `now_wall` or `time.time()` refetch).
      - `test_httpx_downgrade_branch_does_not_increment_error_count` — extracts the `elif is_httpx_transport_error:` branch body via indent-aware regex and asserts the branch logs a `note` AND does NOT touch `error_count` (a regression to `error_count += 1` would have passed the original substring test and silently re-broken §9.3 #20/#21).
      - `test_pgid_present_in_pending_set_during_call` — captures `_PENDING_CHILD_PGIDS` membership DURING `communicate()` so deleting both `add` + `discard` (which would silently disable the atexit safety net) fails loudly.

    Two T3 doc-drift fixes inline:
      - B1-DOC — corrected `_atexit_kill_pending_children` comment to not overpromise OOM/segfault coverage (atexit does NOT run on os._exit/SIGKILL/SIGABRT).
      - B7-DOC — corrected `TestSummariseEventKindMdCellIntegration` docstring (`_md_cell` is invoked by `render_scorecard`, not `parse_iter_result`).

    One T3 cosmetic fix inline:
      - B1-FALLBACK — replaced the post-double-timeout fallback `proc.kill()` with a second `_kill_pgid_safely(pgid)` (idempotent group kill) so a kernel reaping block doesn't leave chromium grandchildren alive.

    Plus B5-BAIL pinned: `test_first_group_failure_does_not_short_circuit_remaining_groups` — the existing partial-failure test only failed the LAST iterated group, so a `if r.status_code != 200: return ...` regression would have passed. Mirror test fails FIRST group, asserts every remaining group still POSTed.

    Total coverage after scrutiny: **136 pytest cases in `tests/test_phase145_run_session.py`** (109 → 136, +27). All phase-145 tests green when isolated to the file.

23. **B2-FLOOR — `child_watch_cap` floor inverts parent/child timeout when `per_iter_timeout < 90`** — **SHIPPED 2026-06-23 (PR-E)**. The historical floor `max(60, per_iter_timeout - 30)` silently produced `child_cap ≥ per_iter_timeout` whenever `per_iter_timeout < 90`, exactly the data-loss mode B2 was meant to prevent. Replaced with a precondition check at function entry: `run_one_iter` raises `ValueError` when `per_iter_timeout < 90`, then computes the cap as `per_iter_timeout - 30` (no floor needed — the precondition guarantees the result is ≥ 60). Old `test_child_watch_cap_floored_at_60s` (which actively pinned the buggy behavior) replaced with two new pins: `test_per_iter_timeout_below_90_raises_value_error` (exhaustively covers 10/30/60/89) and `test_child_watch_cap_at_per_iter_timeout_90_is_60s` (boundary pin). Three pre-existing `TestSubprocessLifecycle` tests using `per_iter_timeout=60`/`10` bumped to 90 with explanatory comment; the FakeProc mock makes the actual wait-time irrelevant.

24. **B1-RACE — stale fake pgids can leak from test failures into atexit handler** — **SHIPPED 2026-06-23 (PR-E)**. New `_snapshot_pending_pgids` autouse fixture on `TestSubprocessLifecycle` snapshots `_PENDING_CHILD_PGIDS` before each test and restores it in `finally`, so a test crash between mock-setup and the production `finally` block can't leak fake pgids (77777/88888/99999/2**30) into module state. Defense-in-depth on top of `_kill_pgid_safely`'s `ProcessLookupError` swallow — the swallow protects against unknown-process false positives, the fixture prevents the unlikely-PID-reuse case from ever reaching the atexit hook.

25. **B1-COVERAGE — three `run_one_iter` recovery branches untested** — **SHIPPED 2026-06-23 (PR-E)**. Added three new pins under `TestSubprocessLifecycle`: (1) `test_run_one_iter_returns_127_on_popen_filenotfound` — when `subprocess.Popen` raises `FileNotFoundError` (broken `sys.executable`, missing module), `run_one_iter` must return `(None, 127, "subprocess spawn failed: …")` rather than propagating the exception and crashing the UAT session. (2) `test_run_one_iter_falls_back_to_proc_pid_when_getpgid_lookups_fail` — when `os.getpgid` raises `ProcessLookupError` (child crashed between Popen and getpgid), `pgid` falls back to `proc.pid` so `_PENDING_CHILD_PGIDS` still has a kill target for atexit. Capture pattern uses an in-call observer in `FakeProc.communicate` (same as `test_pgid_present_in_pending_set_during_call`). (3) `test_run_one_iter_kills_again_when_post_kill_communicate_also_timeouts` — extremely rare path where the post-killpg drain also `TimeoutExpired`; production must call `_kill_pgid_safely` a second time (idempotent group kill) rather than fall back to `proc.kill()` which leaves grandchildren alive.

26. **B2-CLI — `--max-watch-secs` guard only tested at value 0** — **SHIPPED 2026-06-23 (PR-E)**. Renamed `test_max_watch_secs_cli_flag_accepts_value` to `test_max_watch_secs_cli_flag_rejects_non_positive` and parametrized over `[0, -1, -3600]` so a future regression like `if args.max_watch_secs == 0` (strictness drift) is caught. Added behavioral pin `test_max_watch_secs_cli_flag_sets_module_cap_on_positive` which drives `main(["--max-watch-secs", "600", ...])` and intercepts `Api.project` with a sentinel exception so the cap-install branch runs without needing a live daemon; asserts `ps._MAX_WATCH_SECS_CAP == 600` afterward. A regression where the `globals()["..."] = ...` indirection becomes a plain local assignment (silently disabling §9.3 #22 B2) fails here.

27. **B5-NAMING — `test_unexpected_2xx_schema_still_treated_as_cancelled` test name overpromises** — **SHIPPED 2026-06-23 (PR-E)**. Renamed to `test_2xx_classification_is_status_code_only`; AND tightened by making `FakeResp.json` raise `AssertionError` if production ever inspects the body. Status-200 path now uses a separate `FakeStatusResp` class for the `/pipeline/status` GET (where body IS legitimately read), keeping the assertion narrowly scoped to the POST cancel call where the classification contract lives.
...
32. **catalogue data-semantics — `augmented_nodes > total_nodes`** *(root cause pinned 2026-06-25, fix deferred to backend PR)*. The 7812/142 → 5501% production case (§9.3 #31 shipped the cosmetic chip clamp; this is the underlying data inconsistency). `TraceAugmenter._write_manifest` writes two numbers from different scopes: `total_nodes` is current-run work (`result.total_nodes - result.skipped`, which by substitution equals `total_work` — the count of nodes filtered through `_needs_augmentation` THIS run), while `augmented_nodes` is cumulative project-wide (`len(entries)`, where `entries` merges existing on-disk augmentations from `load_existing()` with new ones). After many incremental runs against a fully-augmented project, `len(entries)` grows into thousands while a small-change `total_work` stays in the dozens. See `FINDING_catalogue-augmented-vs-total-semantic-mismatch.md` for the full path trace, three fix options (A: project-wide augmentable denominator [recommended]; B: this-run numerator; C: drop the chip-as-ratio), and the backend PR scope. Rendering-side defense already shipped via PR-M Fixture CA8 (commit `aa8adb7d`).
...
34. **prd-edge-2 — Stats IIFEs omit `promoteForRebuild` while badges include it** — **SHIPPED 2026-06-24 (PR-G + addenda)**. Every stats IIFE / inline-ternary now wraps its raw state with `promoteForRebuild(...)` before passing to `i3SafeStageState(...)`, matching the badge pattern. The running-text branch in every stats site was extended from `safeState === 'running'` to `(safeState === 'running' || safeState === 'rebuilding')` so during a `/pipeline/rebuild` the stats line fires its progress label instead of falling through to the 'Complete' fallback added in PR-F F5. Initially 14 production sites changed; the PR-G adversarial-scrutiny pass (6/6 findings confirmed) then surfaced an addenda: `group_reasoning`'s stats IIFE was switching on RAW flags (`groupReasoningRunning`, `slot_phase`, `running`) directly — the same desync class but missed because its stats text was 'Analyzed' (not 'Not run') and never appeared in the PR-D STAGE_IDS scope. Addenda fix brings group_reasoning into the standard pattern (15 sites total now). Pin tests went 21 → 39 cases including: per-stage promoteForRebuild wrap pin (×15), an **IIFE-body-based** bidirectional sweep that replaces the original line-based sweep (the line-based version was asymmetric — only walked lines containing `'running'`, so removing the 'running' literal entirely silently passed; PR-G scrutiny PRG-TQ-1), and a canonical-`safeState`-name convention pin. Test count after addenda: `statsSafeState.test.ts` 21 → 39 cases.

35. **prd-tq-4 — No behavioural / DOM render test for safeState coercion** — **SHIPPED 2026-06-24 (PR-H, 3 commits: b3ca8d5f infra + dd1ff75c primary test + addenda from scrutiny).** Closed by the first DOM render test in `packages/ui`: `src/components/trace/__tests__/GraphEnrichmentPipeline.behavioral.test.tsx`. Three fixture variants pin the §9.3 #30 contract: (A) primary — `deep_enrichment.phase === 'completed'` + per-stage raw state `not_built`, asserts Deep Knowledge row renders no `'Not run'` AND coerces `data-stage-state='complete'` AND no console.error/warn during render; (B) **gap coverage** — modules.module_count=0 and deepening.total_scored=0 force `group_reasoning` and `clustering` raw states to `'not_built'` so the coercion path is actually exercised for those stages (scrutiny found Fixture A's upstream data short-circuited these to `'complete'` before coercion); (C) **partial payload** — `deepKnowledge` present with `deep_chunks_embedded=0` exercises a different IIFE branch than (A). Three negative controls (`phase='idle'` + the documented `phase='cancelled'`/`'failed'` exception list from freezeGreen.ts:171-173) prove the test responds to phase changes. Adversarial mutation under the /tmp-backup pattern confirms: bypassing `i3SafeStageState` on `group_reasoning` fails Fixture B specifically; disabling the `groupPhase === 'completed'` coercion fails 4 of the 10 assertions (the right ones). Infra wired in PR-H/1: `vitest@^3.0.0`, `happy-dom@^20.10.6` (bumped from `^15.0.0` in PR-H/3 to clear CRITICAL GHSA-37j7-fg3j-429f VM Context Escape + 2 other advisories), `@testing-library/react@^14.3.0`, `@testing-library/jest-dom@^6.4.0`, separate `vitest.config.ts` (not in vite.config.ts to keep vite-plugin-dts out of the test run path), `vitest.setup.ts` (jest-dom global wiring). 204 pre-existing tests confirmed green under the new env in a standalone commit BEFORE the behavioural test landed. 214/214 total green post-addenda. Convention update: the historical "no @testing-library/react" comment in 7+ trace test headers is now stale — the convention is preference-based not infra-constrained (refreshed in PR-H/3).
