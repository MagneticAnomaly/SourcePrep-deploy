...
34. **prd-edge-2 — Stats IIFEs omit `promoteForRebuild` while badges include it** — **SHIPPED 2026-06-24 (PR-G)**. Every stats IIFE / inline-ternary now wraps its raw state with `promoteForRebuild(...)` before passing to `i3SafeStageState(...)`, matching the badge pattern. The running-text branch in every stats site was extended from `safeState === 'running'` to `(safeState === 'running' || safeState === 'rebuilding')` so during a `/pipeline/rebuild` the stats line fires its progress label instead of falling through to the 'Complete' fallback added in PR-F F5. 14 production sites changed in one batch (10 *Stats IIFEs + 4 finalize inline ternaries). Pinned by 15 new vitest cases: per-stage `stats safeState computation wraps raw state in promoteForRebuild` (×14) + sweep `every stats IIFE running-text return also matches safeState === "rebuilding"` (×1). The PR-F F4 atlas-bypass pin was updated to accept the OR form. Test count: `statsSafeState.test.ts` 21 → 36 cases.

35. **prd-tq-4 — No behavioural / DOM render test for safeState coercion** *(still deferred; now tracked for PR-H, the next infra-shaped PR)* — the structural pins in `statsSafeState.test.ts` prove the *syntactic shape* (safeState const present, 'Not run' guarded by safeState, clamps present) but do NOT prove the *behaviour*: that given a fixture where `deep_enrichment.phase === 'completed'` and all per-stage raw computed states are still 'not_built', the rendered Deep Knowledge Embedding row's stats text is NOT 'Not run'. **Infra cost surfaced during PR-G scoping**: `packages/ui` currently has no @testing-library/react, no happy-dom or jsdom, and `vite.config.ts` does not set a vitest `environment`. Adding the behavioural test requires installing those deps + updating vitest config + risk-assessing the impact on the 192 existing trace-tests (which deliberately avoid DOM rendering — see BarrierIndicator.test.tsx comment). That infra work is the natural scope of PR-H; defer #35 until then so PR-G stays bounded to the §9.3 #30 desync fix.
