...
35. **prd-tq-4 — No behavioural / DOM render test for safeState coercion** *(still deferred; now tracked for PR-H, the next infra-shaped PR)* — the structural pins in `statsSafeState.test.ts` prove the *syntactic shape* (safeState const present, 'Not run' guarded by safeState, clamps present) but do NOT prove the *behaviour*: that given a fixture where `deep_enrichment.phase === 'completed'` and all per-stage raw computed states are still 'not_built', the rendered Deep Knowledge Embedding row's stats text is NOT 'Not run'. **Infra cost surfaced during PR-G scoping**: `packages/ui` currently has no @testing-library/react, no happy-dom or jsdom, and `vite.config.ts` does not set a vitest `environment`. Adding the behavioural test requires installing those deps + updating vitest config + risk-assessing the impact on the 136 existing trace-tests (which deliberately avoid DOM rendering — see BarrierIndicator.test.tsx comment). That infra work is the natural scope of PR-H; defer #35 until then so PR-G stays bounded to the §9.3 #30 desync fix.
