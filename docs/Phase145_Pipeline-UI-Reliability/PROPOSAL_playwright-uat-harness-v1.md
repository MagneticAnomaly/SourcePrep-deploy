...
22. **PR-B: harness subprocess + timing hardening** *(deferred from second-pass scrutiny per verifier 2's split recommendation)* — A bundle of 8 items that all touch process lifecycle / clock behavior and don't fit the "phase-vocabulary unification" theme of the PR that just landed. Each item independently verified by the workflow's adversarial completeness audit but bundled into a separate PR because reviewers can't hold both contexts at once.
    - `subprocess.run` in `tools/phase145_uat/run_session.py:833-852` — wrap with `start_new_session=True` and `os.killpg(os.getpgid(p.pid), SIGKILL)` on `TimeoutExpired`; add `atexit.register` fallback. Compounds across iterations: 2 timeouts on a 7-iter run = ~4 orphan headless browsers (~1 GB RSS) surviving until reboot, which on a daemon-RSS-squeezed box cascades into more timeouts.
    - Child `watch_until_idle` budget (`max_seconds=60*60` at `playwright_smoke.py:1256, 1280, 1342, 1401`) exceeds parent SIGKILL (`per_iter_timeout=900`). Parent kills child before `ctx.snap(page, "timeout")` runs, so every parent-timeout iter has no DOM screenshot of the hung state — exactly the evidence needed to triage §9.3 #20. Fix: `max_seconds = per_iter_timeout - 30`.
    - `watch_until_idle` uses `time.time()` for elapsed math at `playwright_smoke.py:852, 882`. Laptop sleep mid-iter (lid closed) causes wall-clock jumps that fire false timeouts and mis-schedule `--update-at-secs`. Switch to `time.monotonic()` for elapsed; keep `time.time()` only for event timestamps.
    - Op-4 httpx timeouts crash teardown at `playwright_smoke.py:929-955` — catch `httpx.TimeoutException` / `HTTPError` specifically and downgrade to `note` event; close `api.http` in `main()`'s `finally`. Closes §9.3 #21.
    - `cancel_and_quiesce` schema + partial-failure tests (e.g. fast_sync=200, deep=500 — currently the per-group outcome is masked in scorecard notes).
    - `parse_iter_result` empty-events.jsonl test (pins the bare-fail empty-notes row class so §9.3 #17 doesn't silently regress).
    - Integration test that `_summarise_event_kind` output survives `_md_cell` escape (pipe in detail text would split the row).
    - Cancel snapshot→post idle→active race in `_pipeline_snapshot` → `cancel_and_quiesce` (narrow window, current 90s budget absorbs it; loop-until-empty fix is a hardening pass).
    Recommended scope: single PR titled "Phase 145 harness process + timing hardening", ~270 LOC. Independent of any other Phase 145 work; can land any time before T5 cadence wiring.
