...
17. **Bare-fail-no-invariant anomaly** *(surfaced by both baseline and post-fix SCORECARDs)* — A row reads `status=FAIL` with all four invariants `✓` AND `summary.json` reports `pass=true, invariant_failure_count=0, desync_count=0, error_count=0`. The harness's notion of "fail" includes "subprocess exited rc != 0" which can fire for reasons orthogonal to invariants/desyncs (Python teardown noise, Playwright shutdown ordering, scheduled-action side-effect codes). 8/12 post-fix iters hit this on Op-3 / Op-4 specifically — strongly correlated with the new T3 `--refresh-at-secs`/`--update-at-secs` codepaths. Trace `playwright_smoke.main` exit paths against `summary.pass`, reconcile run_session's `status` derivation with the summary verdict, OR add a "smoke-rc-vs-summary mismatch" event subtype so the SCORECARD doesn't render legitimate green runs as red rows. Pre-existing in baseline (Op-2 iter 3) but post-fix amplified it because Op-3 / Op-4 now consistently complete cleanly enough to expose the rc/summary skew.
