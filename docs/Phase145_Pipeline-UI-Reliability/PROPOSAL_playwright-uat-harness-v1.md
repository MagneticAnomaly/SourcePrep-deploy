...
23. **B2-FLOOR — `child_watch_cap` floor inverts parent/child timeout when `per_iter_timeout < 90`** *(latent under hostile config; default 900 s unaffected)* — `tools/phase145_uat/run_session.py:868` uses `child_watch_cap = max(60, per_iter_timeout - 30)`. With `per_iter_timeout=10` (which `test_child_watch_cap_floored_at_60s` actively pins), the child is told 60 s while the parent's `proc.communicate(timeout=10)` SIGKILLs the group at 10 s — 50 s BEFORE the child can reach `ctx.snap(page, "timeout")`. This is exactly the data-loss mode B2 was meant to prevent. **Fix:** `child_watch_cap = max(1, min(per_iter_timeout - 30, per_iter_timeout))` or hard-error when `per_iter_timeout < 90`. Update the floor test to match.

24. **B1-RACE — stale fake pgids can leak from test failures into atexit handler** *(defensive; `_kill_pgid_safely` swallows `ProcessLookupError`)* — `TestSubprocessLifecycle` injects fake pgids (77777/88888/99999) into `_PENDING_CHILD_PGIDS`. If a test crashes between Popen-mock setup and the production finally block, those pgids stay in the set; at pytest exit `_atexit_kill_pending_children` runs `os.killpg(88888, SIGKILL)` — `ProcessLookupError` in the common case, but under unlikely PID reuse could SIGKILL an unrelated process group. **Fix:** add an autouse fixture in `TestSubprocessLifecycle` that snapshots/restores `_PENDING_CHILD_PGIDS`.

25. **B1-COVERAGE — three `run_one_iter` recovery branches untested** — (1) `FileNotFoundError` on `Popen` → returns `(None, 127, ...)`. (2) `os.getpgid` raising `ProcessLookupError` → fallback `pgid = proc.pid`. (3) inner `proc.communicate(timeout=5)` after `killpg` also raising `TimeoutExpired` → calls `_kill_pgid_safely(pgid)` again (post-B1-FALLBACK fix). Cheap to add; bundle with B2-FLOOR / B1-RACE.

26. **B2-CLI — `--max-watch-secs` guard only tested at value 0** — production code is `if args.max_watch_secs <= 0`; negative values share the path but aren't exercised. Behavioral check that a positive value actually sets `_MAX_WATCH_SECS_CAP` is missing (`globals()["..."] = ...` indirection could regress to a plain local assignment under refactor and silently revert to no-cap). **Fix:** parametrize the rc=2 test over `[0, -1, -3600]`; add a behavioral check that drives `main(["--max-watch-secs", "600", ...])` and asserts `ps._MAX_WATCH_SECS_CAP == 600`.

27. **B5-NAMING — `test_unexpected_2xx_schema_still_treated_as_cancelled` test name overpromises** — production only branches on `status_code`, never inspects body. The test trivially passes for any 2xx. **Fix:** rename to `test_2xx_classification_is_status_code_only` OR make `FakeResp.json` raise `AssertionError` if called so a future body-parsing regression would fail loudly.

    Items #23-#27 don't fit PR-B's "subprocess + timing hardening" theme; bundle into the next bounded harness-tests PR.
