...
1. All `.codrag/` paths in source go through `src/codrag/core/project_paths.py`. No remaining string literals like `idx_dir / "trace_nodes.jsonl"` outside that module (verifiable by grep, enforced in CI).
...
10. **`.codrag/README.md`** is auto-generated on first migration and kept in sync on subsequent startups.
