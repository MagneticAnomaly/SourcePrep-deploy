...
|:---|:---|:---|
| **Primary distribution** | Direct download from runprep.io | Full control, zero commission, fastest iteration |
...
- **In-app UX:** "Upgrade to Pro" prompts are allowed. Users are directed to
  runprep.io/pricing (Lemon Squeezy checkout) or can enter a license key directly.
...
no api.runprep.io, no machine activation limits — Apple handles all of it.
...
| **Storefront** | payments.runprep.io (Lemon Squeezy-hosted checkout or embedded) |
| **Products** | Monthly Subscription, Perpetual License, Team Subscription (configured as LS products) |
| **Webhooks** | `order_completed` → triggers license generation on api.runprep.io |
...
```
┌─────────────┐     ┌──────────────────┐     ┌──────────────────┐     ┌─────────────┐
│  Customer    │     │  Lemon Squeezy   │     │  api.runprep.io   │     │  Prep App │
...
Note: No Lemon Squeezy, no api.runprep.io, no machine activation logic — Apple handles
...
| 2. **Download** | Goes to runprep.io/download, clicks "Download for Mac" | Browser downloads `.dmg` from GitHub Releases |
...
| 6. **Hit limit** | Tries to add a 4th project | Dashboard shows "Upgrade to Pro" with link to runprep.io/pricing |
| 7. **Buy** | Clicks link → runprep.io/pricing → Lemon Squeezy checkout | LS processes payment, handles tax. Webhook fires. |
| 8. **Receive key** | Gets email with license key | api.runprep.io generates Ed25519 license upon webhook receipt |
| 9. **Activate** | Pastes key into Prep → Settings → License | App calls `POST api.runprep.io/activate` with the LS key |
...
1. User goes to runprep.io/recover (or payments.runprep.io/recover)
...
- ❌ Cannot show "Buy on runprep.io" or "Upgrade to Pro" prompts (unless using US storefront
...
- License key generation (your api.runprep.io service)
- Customer support (support@runprep.io)
...
- Prices adjusted on runprep.io/pricing; LS checkout uses PPP discount codes
...
| **License Generation** | api.runprep.io (serverless) | Ed25519 signing on webhook receipt |
| **Hosting (website)** | Netlify | runprep.io, docs.runprep.io, support.runprep.io, payments.runprep.io (free tier, commercial OK) |
...
| `docs/Phase11_.../LICENSING_IMPLEMENTATION.md` | Activation flow (line 118–122) says "Stripe Webhook → Prep License Service". Should be **Lemon Squeezy webhook → api.runprep.io**. |
...
| api.runprep.io relay service (serverless) | NOT_IMPLEMENTED | — |
...
- No "Visit runprep.io/pricing" link
...
**Bottom line:** Direct download from runprep.io is the correct primary channel.
