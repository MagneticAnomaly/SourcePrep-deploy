...
> **Status:** AUTHORITATIVE — This is the single source of truth for Prep's distribution,
...
> **Product:** Prep (Desktop Application)
...
Prep is distributed as a **native Tauri desktop app** (macOS + Windows).

| Decision | Choice | Rationale |
|:---|:---|:---|
| **Primary distribution** | Direct download from sourceprep.io | Full control, zero commission, fastest iteration |
...
| **NOT used** | pip / PyPI | Prep is not a Python package for end users |
...
- **Website:** sourceprep.io/download
- **Artifacts:**
...
- **Hosting:** GitHub Releases (`github.com/MagneticAnomaly/SourcePrep-MCP/releases`)
  - Download buttons on sourceprep.io/download link to the latest GitHub Release assets
...
- **In-app UX:** "Upgrade to Pro" prompts are allowed. Users are directed to
  sourceprep.io/pricing (Lemon Squeezy checkout) or can enter a license key directly.
...
**What this means:** You CANNOT list Prep as free on the Mac App Store and have
...
| **C4: Two separate apps — "Prep" (free) + "Prep Pro" ($84.99 paid-upfront)** | No IAP complexity, no license key, Apple handles billing, no Lemon Squeezy cut | Two App Store listings to maintain; App Sandbox still applies to both |

**Recommendation: Option C4 if/when Mac App Store is pursued.** Shipping a separate
paid-upfront "Prep Pro" app is fully compliant — the license key prohibition only
applies to unlocking features inside a free app via external payment. A paid-upfront
app has no license mechanism at all: `PREP_TIER=pro` is baked in at build time,
...
no api.sourceprep.io, no machine activation limits — Apple handles all of it.
...
- App Sandbox may conflict with scanning arbitrary repos (Prep's core workflow)
...
| **Storefront** | payments.sourceprep.io (Lemon Squeezy-hosted checkout or embedded) |
| **Products** | Monthly Subscription, Perpetual License, Team Subscription (configured as LS products) |
| **Webhooks** | `order_completed` → triggers license generation on api.sourceprep.io |
...
```
┌─────────────┐     ┌──────────────────┐     ┌──────────────────┐     ┌─────────────┐
│  Customer    │     │  Lemon Squeezy   │     │  api.sourceprep.io   │     │  Prep App │
...
```
Customer pays $84.99 ("Prep Pro" paid-upfront app)
        │
        ▼
...
Note: No Lemon Squeezy, no api.sourceprep.io, no machine activation logic — Apple handles
...
|:---|:---|:---|
| 1. **Discover** | Finds Prep via search, recommendation, or MCP marketplace | — |
| 2. **Download** | Goes to sourceprep.io/download, clicks "Download for Mac" | Browser downloads `.dmg` from GitHub Releases |
| 3. **Install** | Opens `.dmg`, drags to Applications | Standard macOS install. Gatekeeper passes (notarized). |
| 4. **Launch** | Opens Prep from Applications | Tauri app starts, launches Python sidecar daemon on port 8400 |
| 5. **Use (Free)** | Adds 3 projects, connects MCP, uses Prep | Free tier: 3 projects, fully automated syncing. No account needed. |
| 6. **Hit limit** | Tries to add a 4th project | Dashboard shows "Upgrade to Pro" with link to sourceprep.io/pricing |
| 7. **Buy** | Clicks link → sourceprep.io/pricing → Lemon Squeezy checkout | LS processes payment, handles tax. Webhook fires. |
| 8. **Receive key** | Gets email with license key | api.sourceprep.io generates Ed25519 license upon webhook receipt |
| 9. **Activate** | Pastes key into Prep → Settings → License | App calls `POST api.sourceprep.io/activate` with the LS key |
| 10. **Activated** | Sees "Pro — Active" in dashboard | Signed license saved to `~/.runprep/license.key`. Fully offline from here. |
...
1. User goes to sourceprep.io/recover (or payments.sourceprep.io/recover)
...
- ❌ Cannot show "Buy on sourceprep.io" or "Upgrade to Pro" prompts (unless using US storefront
...
- License key generation (your api.sourceprep.io service)
- Customer support (support@sourceprep.io)
...
Prices above are **Band 0 (US/EU/UK/AU/JP)** base prices. Prep uses a 4-band PPP
...
- Prices adjusted on sourceprep.io/pricing; LS checkout uses PPP discount codes
...
| **License Generation** | api.sourceprep.io (serverless) | Ed25519 signing on webhook receipt |
| **Hosting (website)** | Netlify | sourceprep.io, docs.sourceprep.io, support.sourceprep.io, payments.sourceprep.io (free tier, commercial OK) |
...
| `docs/Phase11_.../LICENSING_IMPLEMENTATION.md` | Activation flow (line 118–122) says "Stripe Webhook → Prep License Service". Should be **Lemon Squeezy webhook → api.sourceprep.io**. |
...
| api.sourceprep.io relay service (serverless) | NOT_IMPLEMENTED | — |
...
| App Sandbox blocks Mac App Store (if pursued later) | High — Prep needs filesystem access | Test thoroughly before committing to IAP work |
...
- No "Visit sourceprep.io/pricing" link
...
**Bottom line:** Direct download from sourceprep.io is the correct primary channel.
