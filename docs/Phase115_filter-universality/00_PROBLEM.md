...
CoDRAG's Deep Reasoning stage just spent the night reasoning over storybook-static, node_modules-adjacent build output, and a sea of `.d.ts` declaration files. The filter pipeline *exists* — it's layered (L1 code defaults, L2 repo_policy.json, L3 trace.ignore_patterns) and is *merged at most callsites* — but it has four concrete bugs that collectively defeat universality:

1. **`repo_policy.json` was baked at project-create time** (2026-04-11) with a directory default list that had no entry for `storybook-static`, `codrag_data/**`, `coverage`, `out`, `.turbo`, `.vercel`, etc. New defaults added to the code later are **not** back-propagated — the auto-migrate in `ensure_repo_policy()` only unions **dir-derived** globs, not the `DEFAULT_EXCLUDE_FILE_GLOBS` sequence.
...
4. **Rust walker defaults have drifted.** `codrag-walker/src/lib.rs:89-98` lists only 8 exclude globs against Python's 35+. Rust-path consumers (selfheal) also don't read L3 (`trace.ignore_patterns`) at all.

On top of those, CoDRAG **ingests its own outputs**. `codrag_data/` (daemon-wide SQLite + JSON logs + telemetry) sits at the repo root, isn't in `.gitignore`, and isn't in the default dir-name set. The only thing pushing it out of trace is a single file-glob (`**/codrag_data/ui_config.json`). Everything else under `codrag_data/` is fair game for indexing.
...
`/Volumes/4TB-BAD/HumanAI/CoDRAG/.codrag/repo_policy.json` (created 2026-04-11):
...
Only `DEFAULT_EXCLUDE_DIR_NAMES` is auto-unioned. `DEFAULT_EXCLUDE_FILE_GLOBS` (the `AGENTS.md`, `CLAUDE.md`, `codrag_data/ui_config.json` list) is *never* merged into an existing policy. New file-glob entries we add to the Python source will not reach an old project without a forced re-profile.
...
`engine/crates/codrag-walker/src/lib.rs:89-98`, `WalkConfig::default().exclude_globs`:
...
Eight entries. Python has 35+. The Rust walker is what fast-path indexing uses. It's also what Rust selfheal uses. Selfheal (`codrag-selfheal/src/main.rs:93-103`) reads `repo_policy.json.exclude_globs` and unions with Rust defaults, but **never** reads `project.config.trace.ignore_patterns` (L3 — the user's Knowledge Scope / FolderTree exclusions).
...
CoDRAG writes files. Many of them. Top-level outputs today:

| Path | Writer | Currently excluded? |
|------|--------|---------------------|
| `.codrag/**` | Per-project index | Yes (`**/.codrag/**`) |
| `codrag_data/*.db` | Daemon SQLite stores | **No** |
| `codrag_data/*.json` (roster, telemetry, etc.) | Daemon state | Partially (`ui_config.json` only) |
| `codrag_data/logs/*.log` | Daemon logs | Via `**/*.log` glob, but not dir |
| `AGENTS.md` | `rules_generator.py` | Yes (in DEFAULT_EXCLUDE_FILE_GLOBS) |
| `CLAUDE.md` | User-authored + CoDRAG-managed block | Yes |
| `.cursor/rules/*.mdc` | Per-IDE writers | Yes |

`codrag_data/` is the glaring gap. It's also the daemon-wide store — on the same machine, every project shares it. Indexing it produces a feedback loop: the SQLite journal tables describe every pipeline run; indexing them teaches the LLM about the indexer, not about the repo.

The fix is a central registry — `CODRAG_OUTPUT_DIRS` and `CODRAG_OUTPUT_FILE_GLOBS` in `repo_profile.py` — from which `DEFAULT_EXCLUDE_DIR_NAMES` and `DEFAULT_EXCLUDE_FILE_GLOBS` derive. Any new output CoDRAG adds must be added to the registry; the excludes update automatically. A self-ingestion test (Step 11) guards the invariant.

## Why this is worse than "just a bug"

Three compounding reasons:

- **Universality.** Every CoDRAG user's project has the same issue. The dogfood repo is the canary but fixing the dogfood copy's `repo_policy.json` by hand helps nobody else.
...
- Reorganizing `.codrag/` internals (that is Phase 113).
- Moving `codrag_data/` to `~/.local/share/codrag/` (tracked separately; strengthens tier enforcement but independent of filter correctness).
- UI changes to the Knowledge Scope / FolderTree.

## Acceptance gates for Phase 115

1. `DEFAULT_EXCLUDE_DIR_NAMES` and `DEFAULT_EXCLUDE_FILE_GLOBS` both derive from a central `CODRAG_OUTPUT_*` registry.
...
8. A rebuild of the dogfood repo produces zero `storybook-static/**`, `codrag_data/**`, or `**/*.d.ts` paths in `trace_nodes.jsonl` (integration test).
9. No path matching `CODRAG_OUTPUT_DIRS` appears in `trace_nodes.jsonl` after a full rebuild (regression test).
