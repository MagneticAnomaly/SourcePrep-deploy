...
- `prep_data/` to `~/.local/share/runprep/` tracked as separate work (Track C).

## Step 0 — Add `PREP_OUTPUT_*` registry

**Status:** complete

- Added `PREP_OUTPUT_DIRS = {".runprep", "prep_data"}` at top of `src/prep/core/repo_profile.py`.
...
Verified against the dogfood `.runprep/repo_policy.json`:
...
- `test_rust_walker_covers_prep_output_dirs` — hard invariant: `**/.runprep/**` and `**/prep_data/**` must be in Rust.
...
- `test_user_cannot_silently_remove_prep_output_guard` — even if the user deletes `**/.runprep/**` from `repo_policy.json`, the next load puts it back. Self-ingestion is a hard invariant.
...
Every path that walks the filesystem or reads `trace_nodes.jsonl` now goes through `effective_excludes()`. Self-ingestion guard (`**/.runprep/**`, `**/prep_data/**`) is a hard invariant enforced by `ensure_repo_policy()` — even a direct edit to `repo_policy.json` gets corrected on the next load.
