...
- Added `CODRAG_OUTPUT_DIRS = {".codrag", "codrag_data"}` at top of `src/codrag/core/repo_profile.py`.
- Added `CODRAG_OUTPUT_FILE_GLOBS` tuple with the AI-tool/CoDRAG-generated globs previously inlined in `DEFAULT_EXCLUDE_FILE_GLOBS`.
- Re-derived `DEFAULT_EXCLUDE_DIR_NAMES = CODRAG_OUTPUT_DIRS | { ... }`. Net behaviour change: `codrag_data` now excluded (previously only `.codrag` was).
- Re-derived `DEFAULT_EXCLUDE_FILE_GLOBS = (*CODRAG_OUTPUT_FILE_GLOBS,)`. Net behaviour change: removed `**/codrag_data/ui_config.json` (redundant — whole dir excluded now).
- Verified invariants hold via `.venv/bin/python -c "from codrag.core.repo_profile import ..."`.
...
`engine/crates/codrag-walker/src/lib.rs` — `WalkConfig::default().exclude_globs` expanded from 8 → 57 entries, mirroring Python `DEFAULT_EXCLUDE_DIR_NAMES` (44 dirs as `**/X/**`) + `DEFAULT_EXCLUDE_FILE_GLOBS` (AI-tool + build-artifact globs) + CoDRAG output globs.
...
Build: `cargo build -p codrag-walker --release` — succeeded (1m 36s fresh).
Tests: `cargo test -p codrag-walker --release` — 6/6 pass.
...
- `src/codrag/core/trace/builder.py::TraceBuilder.__init__`: removed the hardcoded `DEFAULT_INCLUDES` / `DEFAULT_EXCLUDES` literals. When callers don't pass globs, TraceBuilder now calls `ensure_repo_policy()` for includes and `effective_excludes()` for excludes. A small safety-net list of language globs remains only as a fallback for the pathological case where the policy produces zero include globs (e.g. profile_repo ran on a repo with no detectable source).
- Added explicit import `from codrag.core.repo_profile import DEFAULT_EXCLUDE_DIR_NAMES` alongside the repo_policy imports (the rest of the file still references it for legacy fallback paths in the walker helpers).
...
- `src/codrag/core/epistemic_enrichment.py::EpistemicEnricher.load_trace_nodes()` now resolves `effective_excludes()` and drops any node whose `path` matches. Logs a WARNING with the drop count so stale trace_nodes.jsonl (from a pre-Phase-115 build) is visibly reconciled on first load post-upgrade.
...
Smoke test: `from codrag.core.epistemic_enrichment import EpistemicEnricher; inspect.getsource(EpistemicEnricher.load_trace_nodes)` confirms `effective_excludes` + `pathspec` wired in.

## Step 7 — Fix Rust selfheal L3 gap

**Status:** complete

`engine/crates/codrag-selfheal/src/main.rs` now accepts an optional `--extra-excludes-file <path>` argument. The file holds a JSON array of patterns sourced from `project.config.trace.ignore_patterns`; selfheal merges them onto the L1+L2 set at startup.
...
Build: `cargo build -p codrag-selfheal --release` — clean (4.5s).
Tests: `cargo test -p codrag-selfheal --release` — 0 tests, pass.

## Step 8 — Audit watcher filter merge

**Status:** complete

`src/codrag/core/watcher.py::AutoRebuildWatcher._load_policy_globs()` now returns excludes from `effective_excludes()` instead of reading `pol["exclude_globs"]` directly. Net behaviour identical post-Step-3 (ensure_repo_policy auto-merges L1 into L2 on disk), but the watcher now shares the single resolver with TraceBuilder and epistemic_enrichment. Inline comment documents the L3 gap — watcher has no project-config access, so runtime `trace.ignore_patterns` can drift. Tracked as future work.
...
- `test_trace_builder_does_not_enumerate_leak_culprits` — end-to-end via `TraceBuilder._enumerate_files()`. Zero culprits returned; the legit `src/codrag/core/trace/builder.py` control file still surfaces.
...
Added `src/codrag/core/trace/loaders.py::load_filtered_trace_nodes()` as the shared reader contract — applies `effective_excludes()` + pathspec at read time. Rewired four callers:

- `src/codrag/core/epistemic_enrichment.py::EpistemicEnricher.load_trace_nodes` — now a 3-line wrapper.
- `src/codrag/core/augmenter.py::TraceAugmenter.load_trace_nodes` — **BLOCKER** pre-fix: this was feeding unfiltered nodes into Pass 1 augmentation. Pre-Phase-115 leak paths could be re-enriched every build.
- `src/codrag/api/routers/projects/search.py::_load_trace_nodes_for_project` — **BLOCKER** pre-fix: unfiltered LOD context for structured search results could include storybook-static / *.d.ts content.
- `src/codrag/core/atlas/role_projection.py::_load_trace_nodes` — fallback path; derives repo_root from on-disk policy since the caller only has `index_dir`.

L3 status (clarified, not a bug):
- `src/codrag/services/pipeline/workers.py:174-179` already unions `project.config.trace.ignore_patterns` into the `exclude_globs` it passes to `TraceBuilder`. Pipeline runs honour L3 today.
...
Noise cleanup:
- `src/codrag/core/watcher.py::_extra_exclude_globs` — dropped the hardcoded `**/.codrag` triple (now redundant with L1). Kept the `index_dir`-relative guards for projects that use a non-standard index location outside the CoDRAG-owned dir names.
...
- L1 (code defaults) — `repo_profile.DEFAULT_EXCLUDE_DIR_NAMES` + `DEFAULT_EXCLUDE_FILE_GLOBS` + `CODRAG_OUTPUT_*` registry.
