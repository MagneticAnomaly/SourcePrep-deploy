# Boundary Analysis: Prep vs. Paperclip — Who Owns What?

*2026-04-06 — Eric's Prep MCP R&D*

## The Question

Where does agent staffing live? Prep has the codebase knowledge. Paperclip has the agent runtime. The agent builder script sits in the middle. Who does what?

---

## What Already Exists (I Read All of This)

### In Prep (`src/prep/agents/`)
...
| **`core.py`** (AgentCore) | Unified facade: Prep read + Paperclip write + Git + LLM + Agent CRUD | Built |
...
| Component | What It Does | Status |
|-----------|-------------|--------|
| **`paperclip-plugin-prep/`** | Full Paperclip plugin manifest with 5 tools, 4 UI slots (dashboard widget, agent knowledge scope tab, issue context tab, settings page), scheduled reindex job | Built |
| **`paperclip-skill/`** | Claude Code skill installer that symlinks Prep tools into `~/.claude/skills/prep` | Built |
...
- **What agents get:** `prep`, `prep_search`, `prep_impact`, `prep_audit`, `prep_observe`, `prep_concepts`
- **Who uses it:** Any AI agent in any IDE. Not Paperclip-specific.
- **Staffing relevance:** The `role=` parameter on `prep()` and `prep_search()` filters context per agent. Prep's `AgentScopeManager` handles the masking.

### Surface 2: Paperclip Plugin (`@prep/paperclip-plugin`)
- **What it adds:** 5 registered tools (`prep:context`, `prep:search`, etc.), dashboard widgets, agent knowledge scope tab, issue context tab, settings page, scheduled reindex job
- **Who uses it:** Paperclip agents during heartbeat runs
- **Staffing relevance:** The plugin gives agents **access** to Prep during runs. But it does NOT currently expose the HR system.

### Surface 3: Paperclip Runtime (heartbeat lifecycle)
- **What it controls:** Task assignment, approval gates, budget, agent execution
- **Staffing relevance:** Agents are CREATED and MANAGED here. Prep's `push_to_paperclip()` pushes role specs INTO Paperclip via HTTP.

---

## The Answer: Prep Generates, Paperclip Operates, HR Bridges

The existing code already implements your ideal model:

### 🟢 Prep owns GENERATION (the "hiring")
...
**This should be a dashboard button.** Prep has the deep structural knowledge (atlas, modules, hub files, domain tags, architecture layers) that makes role generation meaningful. A "Generate Staff" button in the Prep dashboard is the natural home.
...
3. Agents call `prep:*` tools during their runs via the plugin

### 🟡 The HR Agent bridges ongoing adjustment

An HR agent **running inside Paperclip** but **powered by Prep data** handles:
...
This is already designed in Prep's `hr/engine.py` — it just needs to be wired up.
...
**Recommendation:** Keep the script as a Claude Code workflow for manual team design, but invest in exposing the StaffingEngine through the Prep dashboard UI for the "push-button" experience.
...
The plugin **already** does something substantial — it gives agents Prep tools during Paperclip runs. But it could do more:
...
|---------------|-------|-----|
| **Structural analysis** (index, trace, atlas, modules) | Prep daemon | Has the graph engine |
| **Role generation** (readiness, inference, AGENTS.md) | Prep daemon (StaffingEngine) | Needs atlas + modules + LLM |
| **Role persistence** (roster) | Prep daemon | Lives alongside project index |
| **Drift detection** (fitness scoring) | Prep daemon | Needs current graph vs. stored roles |
| **Dashboard UI** for generation | Prep dashboard | Users push a button, see results |
| **Push to Paperclip** (agent CRUD) | Prep daemon → Paperclip API | `push_to_paperclip()` is already built |
| **Agent execution** (heartbeat, tasks, budget) | Paperclip | Its core job |
| **Tool access during runs** | Paperclip plugin | Routes `prep:*` calls to daemon |
| **Ongoing role adjustment** | HR agent in Paperclip + Prep data | Bridges the gap |
...
1. **Expose StaffingEngine via Prep API** — Add `/projects/{id}/staff` endpoints for readiness, generate, drift, push
2. **Add dashboard panel** — "Agent Staff" tab in Prep dashboard with Generate/Audit/Push buttons

### Medium-term
3. **Wire HR agent to Paperclip** — The HR agent runs in Paperclip on a schedule, calls Prep's drift detection, and auto-regenerates drifted roles
...
6. **Evaluate whether the plugin should surface the org chart** — Is that a Paperclip UI concern or a Prep UI concern?
