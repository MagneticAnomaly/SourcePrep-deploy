...
However, Phase 64 introduced synchronous processing inside the Atlas Worker that generates `.runprep/atlas_role_*.md` files and `atlas_segments_manifest.json`. If the pipeline halts *between* the main atlas and the segment/role atlases, the resume logic completely abandons the Sub-Atlases.
...
**Workflow Steps:**
1. **Determine Role Profile:** Read `AGENTS.md` or `.runprep/project.json` to get the defined instructions and overarching goal of the `{agent_role}` (e.g. `Backend Architect`).
