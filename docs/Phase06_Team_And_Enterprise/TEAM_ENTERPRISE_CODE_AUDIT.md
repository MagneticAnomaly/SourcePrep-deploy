...
| File | Lines | Role |
|------|-------|------|
| `src/prep/services/headless_runner.py` | 618 | Core headless pipeline orchestration |
| `src/prep/services/s3_storage.py` | 313 | S3-compatible upload/download with atomic swap |
| `src/prep/services/remote_sync.py` | 382 | Client-side polling, delta pruning, secrets detection |
| `src/prep/core/layered_index.py` | 366 | Merged remote + local delta search |
| `src/prep/core/team_config.py` | 155 | Team config schema (enforcement, policy, models) |
| `src/prep/core/feature_gate.py` | 214 | License tier gating |
...
**Issue:** Neither Dockerfile includes a `USER` directive. All processes (Prep, Ollama, git clone) run as root inside the container. If a malicious repo is cloned and exploits a vulnerability in the parser or git, the attacker has root access inside the container.
**Fix:** Add a non-root user and switch to it after installation.

### SEC-2: CLI Accepts Secrets as Command-Line Arguments
**File:** `src/prep/cli.py` lines 1196, 1202-1203
...
**File:** `src/prep/services/headless_runner.py` lines 438-463
**Risk:** LOW — Confusing dual-gate architecture.
**Issue:** The CLI entry point (`cli.py`) enforces a hard gate via `check_feature("team_config")`. But `HeadlessRunner._verify_license()` implements a completely separate, softer check using `PREP_LICENSE_KEY` env var with prefix matching. These two systems don't interact. The soft gate in the runner checks a different env var than the feature gate system.
...
**Files:** `src/prep/core/team_config.py` vs `src/prep/services/remote_sync.py`
...
The `prep-sync.yml` uses `concurrency` with `cancel-in-progress: true` to prevent stale builds from piling up. It supports both CPU+BYOK and GPU webhook modes with a simple toggle. Secrets are properly injected via GitHub Secrets.
