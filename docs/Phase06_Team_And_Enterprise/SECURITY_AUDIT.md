# Prep Team/Enterprise — Deep Security Audit
...
Prep's Team/Enterprise layer has a unique attack surface because it:
...
### Attacker Profiles
- **Malicious Repo Author:** Crafts a repository designed to exploit Prep during indexing
...
**File:** `src/prep/core/feature_gate.py` lines 126-150
**CVSS Estimate:** 8.0 (High — Revenue Impact)
**Attack:** Any user can bypass all paid features by:
  1. Creating `~/.runprep/license.json` with `{"tier": "enterprise"}`, OR
  2. Setting `PREP_TIER=enterprise` in their environment
...
**Recommendation:**
  - **Phase 1 (Ship Blocker):** Implement Ed25519 signature verification. The license file should contain a `signature` field signed by Prep's private key. `get_license()` must verify the signature against the embedded public key before trusting the tier.
  ...
  - **Dev Override:** Keep `PREP_TIER` env var but ONLY when a separate `PREP_DEV_MODE=1` flag is also set, and log a prominent warning.

### CRIT-2: S3 Endpoint URL is Attacker-Controlled (SSRF via team_config.json)
**File:** `src/prep/services/remote_sync.py` lines 211-214
**CVSS Estimate:** 7.5 (High — SSRF)
**Attack:** The `s3_endpoint` field in `.runprep/team_config.json` is committed to Git. A malicious contributor can submit a PR that changes `s3_endpoint` to an internal network address (e.g., `http://169.254.169.254/latest/meta-data/` on AWS, or `http://internal-service.corp:8080/`). When any team member's Prep daemon starts polling, it makes HTTP requests to the attacker-controlled endpoint using the developer's network context, potentially:
...
**File:** `src/prep/services/headless_runner.py` lines 485-503
...
**File:** `src/prep/services/remote_sync.py` lines 102-113
**Attack:** `.runprep/.secrets` contains S3 credentials in plaintext JSON. If the file permissions are too open (e.g., 644 instead of 600), any local user on a shared machine can read the credentials.
...
**File:** `src/prep/services/headless_runner.py` line 458
...
**File:** `src/prep/services/s3_storage.py` line 165-167
**Attack:** The `prefix` field is user-controlled (from team_config.json or CLI). A malicious prefix like `../../other-team-project` could cause Prep to read/overwrite another team's index in the same S3 bucket.

**Recommendation:** Validate that `prefix` does not contain `..` segments or absolute paths.

### HIGH-5: Zip Bomb Denial of Service
**File:** `src/prep/services/s3_storage.py` lines 288-294
...
**File:** `src/prep/services/remote_sync.py` line 52
...
**File:** `src/prep/services/s3_storage.py` lines 268-312
...
**File:** `src/prep/core/layered_index.py` line 217
...
**File:** `src/prep/services/headless_runner.py` lines 439-464
...
**File:** `src/prep/services/s3_storage.py` line 131
...
*Added: Mar 9, 2026*
*Scope: All `src/prep/` — API server, LLM client, core modules, embedder, API routers*
...
- **IPC token auth middleware** exists in `server.py` via `PREP_DAEMON_TOKEN` env var — when set, all API calls (except `/health` and `/events`) require `Authorization: Bearer <token>`

## 🟡 Full-Codebase Findings

### FULL-1: CORS `allow_origins=["*"]` with `allow_credentials=True`
**File:** `src/prep/server.py` lines 94-100
**Risk:** MEDIUM
**Issue:** The combination of `allow_origins=["*"]` and `allow_credentials=True` means any website can make credentialed cross-origin requests to the Prep daemon. While the daemon runs locally and the IPC token provides protection when set, if `PREP_DAEMON_TOKEN` is NOT set (the default for development), any malicious website visited by the developer could call Prep API endpoints.
...
**File:** `src/prep/api/routers/llm.py` lines 50-58, 592-709
...
**File:** `src/prep/core/llm_client.py` line 591
**Risk:** LOW
...
**Context:** This is Google's official API design — all Gemini REST API calls use query-parameter auth. There's nothing Prep can do differently here without switching to Google's OAuth flow.
...
**File:** `src/prep/server.py` (global)
...
**Context:** Prep is a local daemon, so the attack surface is limited. The IPC token provides access control. Rate limiting is more relevant for the future Prep Manager (Team server).
**Recommendation:** Defer to Prep Manager (Phase 06 future). For the local daemon, the IPC token is sufficient.

### FULL-5: Git Log Subprocess in Inferred Edges
**File:** `src/prep/core/inferred_edges.py` line 852
**Risk:** LOW
...
| ID | Status | Fix |
|----|--------|-----|
| FULL-1 | ✅ Fixed | CORS restricted to `localhost`, `127.0.0.1`, and `tauri://` origins. Dev escape hatch via `PREP_CORS_ALLOW_ALL=1` env var. |
...
The core Prep codebase has a **good security baseline**:
- No deserialization vulnerabilities (pickle, yaml, eval)
- No shell injection vectors
- IPC token auth when `PREP_DAEMON_TOKEN` is set
