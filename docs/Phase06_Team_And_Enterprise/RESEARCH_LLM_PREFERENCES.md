...
When planning the CoDRAG headless indexing architecture for teams, we must anticipate how businesses prefer to spend their compute and LLM budgets. Do they want to rent GPUs and run open-source models (Qwen3), or do they want to run cheap CPU servers and pay OpenAI/Anthropic per token?
...
**What they will do:** 
- They will run the `codrag-headless` container directly inside a standard GitHub Action or GitLab CI runner (which are CPU-only and essentially free).
- They will configure CoDRAG to use their existing Anthropic/OpenAI API keys (`gpt-4.1-mini` or `claude-3.5-haiku`) for the 10-stage enrichment pipeline.
...
**What they will do:**
- They will deploy the `codrag-headless` template to RunPod, Modal, or AWS SageMaker, renting an Ada 5000, A4000, or A10G GPU.
- They will configure CoDRAG to use the embedded Ollama engine with `qwen3:8b` or `deepseek-coder-v2`.
...
## 2. Strategic Implications for CoDRAG

Because both profiles exist in large numbers, the `codrag-headless` build must seamlessly support both paradigms without requiring different Docker images.
...
```
*(Note: CoDRAG's native ONNX embedder runs flawlessly on CPU, so semantic search generation doesn't require a GPU, only the LLM reasoning stages do).*
...
**Conclusion:** 
Teams will start with the CPU + BYOK approach because it requires zero infrastructure setup. As their codebase grows and their OpenAI bill spikes, they will migrate to the RunPod/Modal GPU approach. CoDRAG must provide the templates to make that transition painless.
