...
| Feature | Description | Priority | Status |
|---|---|---|---|
| **Team Config** | Shared `.codrag/team_config.json` in repo | HIGH | ✅ Built (`team_config.py`) |
...
```
.codrag/team_config.json (committed to repo, secret-free)
    │
    ▼
...
| License file at `~/.codrag/license.json` | ✅ Built | `src/codrag/core/feature_gate.py` |
...
User buys on codrag.io/pricing
  ...
  → App saves to ~/.codrag/license.json
...
4. User local settings (~/.codrag/settings)
5. Built-in defaults
```
...
- Recommended EDR exclusion paths (`.codrag/`, index directories)
...
| Check | What it verifies | Source |
|---|---|---|
| **Secrets file permissions** | `.codrag/.secrets` is mode 600 (owner-only) on Unix | Local file check |
...
**CoDRAG relevance:** CoDRAG has `.codrag/team_config.json` and `.windsurf/workflows/*.md` — both are config files read by AI tools. A malicious contributor could inject hidden Unicode instructions.
...
- **EA-I13**: Unicode sanitization check — flag `team_config.json` or `.codrag/` files with invisible Unicode
