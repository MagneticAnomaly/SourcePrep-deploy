...
- A repo is available with:
  - `.runprep/team_config.json` committed
  - optional `.runprep/index/**` committed (for T1)
...
### Setup
On machine A:
- Ensure embedded mode index exists at `{project_root}/.runprep/index/`.
...
- No `.runprep/**` watcher loops occur during project open.
...
### Goal
If conflict markers exist inside `.runprep/index/**`, the index is invalid and rebuild is required.

### Steps
1. Create a merge conflict inside `.runprep/index/**` (synthetic is fine):
