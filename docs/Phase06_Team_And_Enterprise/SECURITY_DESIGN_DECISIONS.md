...
Right now, anyone can create a file at `~/.codrag/license.json` with the contents `{"tier": "enterprise"}` and unlock every paid feature for free. There is zero verification that the person actually paid.
...
7. CoDRAG saves the activation response to ~/.codrag/license.json
...
### The Problem
The `s3_endpoint` field in `.codrag/team_config.json` is committed to Git. A malicious PR could change it to an internal network address, and every developer who pulls would have their CoDRAG daemon make requests to the attacker's endpoint — with S3 credentials attached.
...
### The Question
Should we check file permissions on `.codrag/.secrets`? Would it ever leave the developer's machine? Should it be excluded from the trace graph?
...
CoDRAG's `.codrag/.secrets` lives inside `.codrag/` which is typically gitignored. But we should also ensure `.secrets` is explicitly in our recommended `.gitignore` patterns. This file should **never** leave the developer's machine via Git.

**Layer 2: Always Excluded from Trace Graph (Should Implement)**
The Rust parser (`codrag-parser`) should never index `.codrag/.secrets`. Our default `exclude_globs` should include `.codrag/.secrets` and `**/.secrets`. Even if someone explicitly includes `.codrag/` in their trace scope, the secrets file must be excluded. This is a hard exclude — not configurable.

**Layer 3: Permission Check on Unix Only (Should Implement)**
Follow the SSH model:
- **On Unix/macOS:** When reading `.codrag/.secrets`, check if permissions are wider than `0600`. If they are, log a `WARNING` but still read the file (unlike SSH which hard-refuses). This is gentler because CoDRAG is a dev tool, not a security-critical system.
...
When the documentation or CLI creates `.codrag/.secrets` for the first time, explicitly set `chmod 600` on Unix:
```python
import os, stat
...
3. The secrets file is in `.codrag/` not in the source tree
