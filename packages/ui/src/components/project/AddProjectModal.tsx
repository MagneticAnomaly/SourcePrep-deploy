...
const INDEX_LOCATION_INFO: Record<ProjectMode, { best: string; detail: string }> = {
  standalone: {
    ...
    detail: 'Index lives in the app data directory (~/.sourceprep/). Move or delete the repo without losing your index. Ideal if the repo is on network or external storage.',
  },
  embedded: {
    best: 'Best when boot disk is faster',
    detail: 'Index lives inside the repo at .sourceprep/. Co-located with your code for zero-config. Best when your boot SSD is faster than the repo\'s storage device.',
  },
  custom: {
...
export function AddProjectModal({
  ...
  const modes: { key: ProjectMode; label: string; icon: typeof Folder; desc: string }[] = [
    ...
    { key: 'embedded',   label: 'Embedded',   icon: Layout,    desc: '.sourceprep/ in repo' },
