...
const tandemTools = [
  { id: 'cursor', name: 'Cursor', category: 'AI IDE', desc: 'Connect SourcePrep via MCP to give Cursor perfect project-wide context without copying files.' },
  { id: 'windsurf', name: 'Windsurf', category: 'AI IDE', desc: 'Windsurf agents use SourcePrep to autonomously navigate the trace graph before editing.' },
  { id: 'cline', name: 'Cline / Roo', category: 'VS Code Extension', desc: 'Stop dropping raw files into context. Give your agent the SourcePrep LOD capsule instead.' },
    ...
    features: [
      {
        ...
        competitors: {
          ...
          grepai: {
            text: 'Text Index',
            status: 'none',
            detail: "Grepai builds a basic text index for semantic search but doesn't parse code structure at all. There are no call-graph edges, no containment relationships, and no module boundaries. It's a powerful search tool, but SourcePrep provides full structural understanding on top of semantic search.",
          },
          bloop: {
            text: 'Rust\n(Tree-sitter)',
            status: 'full',
      ...
      },
      {
        ...
        competitors: {
          gitnexus: {
            text: 'Web UI\n/Terminal',
            status: 'partial',
            detail: "GitNexus offers a web UI and terminal interface for browsing the graph. The web UI is functional and shows precomputed clusters and wiki documentation. It's less purpose-built for context-inspection than SourcePrep's dashboard but provides reasonable visibility into the knowledge graph.",
          },
          vexp: {
            text: 'VS Code\nOnly',
            status: 'partial',
      ...
      },
      {
        ...
        competitors: {
          ...
          vexp: {
            text: 'Graph Centrality\nin Ranking',
            status: 'partial',
            detail: "Vexp incorporates graph centrality into its search ranking. Similar concept to SourcePrep's edge weights but not user-configurable. The ranking is purely algorithmic with no developer input on priorities.",
          },
          empirica: {
            text: 'N/A',
            status: 'none',
      ...
      },
      {
        ...
        competitors: {
          ...
          vexp: {
            text: 'Local\n(VS Code Extension)',
            status: 'full',
            detail: "Vexp is fully local-first, running entirely within VS Code. No cloud calls. SQLite storage stays on disk. Strong privacy story \u2014 comparable to SourcePrep's approach. The main difference is SourcePrep works across any IDE via MCP and can run headlessly.",
          },
          empirica: {
            text: 'Git-Native\n(LLM calls needed)',
            status: 'partial',
          ...
          grepai: {
            text: 'Privacy-First\nLocal',
            status: 'full',
            detail: "Grepai is explicitly privacy-first with local embeddings. Strong privacy story, comparable to SourcePrep's approach. Both tools keep everything on-device with zero cloud dependencies for the core functionality.",
          },
          bloop: {
            text: 'Local\n(Qdrant Instance)',
            status: 'full',
    ...
    <div className={`w-full ${className}`}>
      ...
      <div className="hidden lg:block relative w-full mb-8">
        ...
        <div className="flex w-full rounded-xl border border-border bg-background shadow-xl overflow-hidden">
          {/* Fixed left columns: Category | Feature | SourcePrep */}
              ...
              <thead>
                <tr>
                  ...
                  <th className="w-[130px] min-w-[130px] bg-[#0c1222] border-b border-l border-primary/30 p-3 text-center align-bottom">
                    <div className="inline-flex items-center justify-center px-3 py-1 rounded-lg bg-primary border border-primary-hover text-background font-bold text-base mb-1 shadow-lg shadow-primary/20">SourcePrep</div>
                    ...
                    {category.features.map((feature, featIdx) => (
                      <tr key={feature.id}>
                        ...
                        <td className="bg-[#0c1222] border-b border-l border-primary/30 p-3 text-center align-top cursor-help"
                          onMouseEnter={(e) => showTooltip(e, feature.prep.detail, feature.name, 'SourcePrep')}
                  ...
                  <div key={feature.id} className="bg-surface border border-border rounded-xl overflow-hidden shadow-sm flex flex-col">
                    ...
                    <div className="p-4 bg-[#0c1222] border-b border-primary/30 shadow-inner">
                      <div className="flex items-start gap-3 mb-3">
                        ...
                        <div>
                          <div className="text-xs font-bold text-primary uppercase tracking-wider mb-1">SourcePrep</div>
            ...
            <table className="w-full text-left border-collapse min-w-[600px] text-xs">
              <thead>
                <tr>
                  ...
                  <th className="bg-[#0c1222] p-3 border-b border-border text-center font-bold text-primary whitespace-nowrap">SourcePrep</th>
                  {competitors.map(c => (
