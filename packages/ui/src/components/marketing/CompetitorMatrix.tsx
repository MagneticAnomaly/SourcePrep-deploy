    ...
    features: [
      {
        ...
        competitors: {
          ...
          vexp: {
            text: 'Capsule\nContext',
            status: 'full',
            detail: "Vexp implements capsule context very similarly to CoDRAG \u2014 full source for pivot nodes, signatures for neighbors. Credit where it's due: this is one of the closest approaches to CoDRAG's LOD system and validates the core idea. The difference is CoDRAG's dual-engine compression (LOD for code, LLMLingua-2 for docs) and module-summary injection, which provide additional layers of context beyond what Vexp includes, plus CoDRAG's dashboard lets you visually inspect the assembled capsule before it's sent.",
          },
          empirica: {
            text: 'Reasoning\nCheckpoints',
            status: 'partial',
      ...
      },
      {
        ...
        codrag: {
          text: 'Dual-Engine\nCompression (3\u201320x)',
          status: 'full',
          detail: "CoDRAG achieves 3\u201320x token compression through a dual-engine approach: LOD-based structural compression for code (signatures instead of full bodies) and LLMLingua-2 token pruning for documentation (~2.4\u00d7). The compression level adapts dynamically per query and per client tier \u2014 Claude/Gemini get more full-source files, local models get tighter compression to fit constrained windows.",
        },
        competitors: {
          gitnexus: {
            text: 'High\n(via Precomputation)',
          ...
          vexp: {
            text: 'High\n(Signature Only)',
            status: 'partial',
            detail: "Vexp achieves good efficiency by returning only signatures for non-focal nodes. This is the same core strategy as CoDRAG's LOD system, and it works well. Vexp's compression is query-adaptive and effective. CoDRAG's additional edge comes from dual-engine compression (LOD for code, LLMLingua-2 for docs), module-summary injection, tier-adaptive LOD thresholds, and the BM25+semantic scoring that better prioritizes which nodes to include at all.",
          },
          empirica: {
            text: 'Low\n(State Dumps)',
            status: 'none',
