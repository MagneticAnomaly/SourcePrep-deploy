import type { Meta, StoryObj } from '@storybook/react';
import { AnimatedCLI } from '../../components/console/AnimatedCLI';
import { prepSearchDemo, prepImpactDemo, prepOverviewDemo } from '../../components/console/demo-scripts';

const meta: Meta<typeof AnimatedCLI> = {
  title: 'Console/AnimatedCLI',
  component: AnimatedCLI,
  parameters: {
    layout: 'padded',
    docs: { description: { component: 'Auto-animated terminal playback showing SourcePrep MCP tool calls in action.' } },
  },
};

export default meta;
type Story = StoryObj<typeof AnimatedCLI>;

/** Semantic search demo — shows prep_search in action */
export const SemanticSearch: Story = {
  args: {
    script: prepSearchDemo,
    autoPlay: true,
    theme: 'dark',
  },
};

/** Impact analysis demo — shows prep_impact */
export const ImpactAnalysis: Story = {
  args: {
    script: prepImpactDemo,
    autoPlay: true,
    theme: 'dark',
  },
};

/** Overview demo — shows prep ambient context */
export const ProjectOverview: Story = {
  args: {
    script: prepOverviewDemo,
    autoPlay: true,
    theme: 'dark',
  },
};

/** Claude theme variant */
export const ClaudeTheme: Story = {
  args: {
    script: prepSearchDemo,
    autoPlay: true,
    theme: 'claude',
  },
};
