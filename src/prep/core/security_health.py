...
Usage:
    from prep.core.security_health import run_security_checks
...
def _check_license() -> Dict[str, Any]:
    ...
    try:
        from prep.core.feature_gate import get_license
        lic = get_license()
        issues: List[str] = []
        ...
        if lic.expires_at:
            from prep.core.feature_gate import _is_license_expired
...
def _check_dlp_compliance(project_root: Optional[Path]) -> Dict[str, Any]:
    ...
    try:
        from prep.core.team_config import load_admin_policy
...
def _check_content_sanitization() -> Dict[str, Any]:
    ...
    try:
        from prep.core.content_sanitizer import sanitize_llm_input, sanitize_output, validate_llm_output, normalize_nfkc
...
def _check_secret_detection_coverage(project_root: Optional[Path] = None) -> Dict[str, Any]:
    ...
    """
    try:
        from prep.core.content_sanitizer import BUILTIN_SECRET_PATTERNS, detect_secrets
