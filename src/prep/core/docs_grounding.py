...
# Phase 133b: markers that identify a file as prep-self-output. The concept
# synthesizer's grounding load skips files containing any of these in their
# leading content — including them would feed prep's own documentation back
# as planning material (the sample showed `.cursor/rules/prep.mdc` and
# `.claude/skills/prep.md` are the dominant content across most repos, with
# headers like "SourcePrep structural codebase intelligence" or auto-generated
# AGENTS.md carrying `<!-- prep-opportunities-start -->`). Without this
# filter, "include agent-instruction dirs for concepts" mostly grounds on
# self-reference. Project-specific custom skills (e.g.,
# `.claude/skills/playwright-smoke/SKILL.md`) lack these markers and pass
# through.
PREP_SELF_OUTPUT_MARKERS: tuple[str, ...] = (
    # "Auto-generated by ..." family — covers rules_generator, staffing
    # agent KNOWLEDGE.md, etc.
    "Auto-generated by Prep",
    "Auto-generated by SourcePrep",
    "Auto-generated by RunPrep",
    "Auto-generated by CoDRAG",
    "Auto-generated by Prep Staffing",
    # rules_generator output (`.cursor/rules/prep.mdc`, `.claude/skills/prep.md`)
    ...
    # Trimmed — the rules_generator emits both "from Prep" (no "tools" word
    # on same line) and "from SourcePrep tools" depending on YAML wrap.
    "Get structural codebase context from Prep",
    "Get structural codebase context from SourcePrep",
    # HTML-comment markers used to splice managed sections into user-owned
    # docs (AGENTS.md, etc.)
    "<!-- prep-managed-start -->",
    "<!-- prep-opportunities-start -->",
    # Staffing agent's per-role generation flow — SOUL.md / AGENTS.md
    # files in `.agents/<role>/` start with a `<think>` block whose
    # content references the file being generated. The exact wording
    # varies ("user wants me to generate a SOUL.md", "user wants a
    # SOUL.md identity file", etc.) so we match on the distinctive
    # phrase that always appears regardless of phrasing.
    "SOUL.md identity file",
    "AGENTS.md instruction file",
)

# Number of leading characters to scan for prep-self-output markers.
# Markers conventionally appear in the first 200-500 chars of generated
# files (frontmatter or top-of-file comment). 1000 is generous and still
# bounded for large files.
_SELF_OUTPUT_SCAN_HEAD_CHARS = 1000


def _looks_like_prep_self_output(body: str) -> bool:
    """True when the file's leading content matches any prep-self-output marker.

    Pure helper. Called by ``discover_planning_docs`` to skip prep-generated
    files when assembling concept-grounding doc set.
    """
    if not body:
        return False
    head = body[:_SELF_OUTPUT_SCAN_HEAD_CHARS]
    return any(marker in head for marker in PREP_SELF_OUTPUT_MARKERS)
    ...
    *,
    ...
    for md_abs in md_files:
        ...
        try:
            ...
            continue
        # Phase 133b: skip prep-self-output. These files (e.g.,
        # `.cursor/rules/prep.mdc`, auto-generated AGENTS.md) carry markers
        # like "SourcePrep structural codebase intelligence" or
        # `<!-- prep-managed-start -->` in their head. Including them in
        # concept grounding produces meta-recursive output. Genuine
        # project-specific skills in the same dirs (no markers) pass through.
        if _looks_like_prep_self_output(body):
            continue
