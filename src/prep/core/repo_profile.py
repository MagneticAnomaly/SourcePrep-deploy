...
DEFAULT_EXCLUDE_DIR_NAMES: Set[str] = PREP_OUTPUT_DIRS | {
    ...
    ".agents",
    ".opencode",
    # IDE / editor state — personal preferences, not architecture.
    # `.vscode/` is borderline (some teams share `launch.json`); default
    # is exclude per Phase 133b research §9.
    ".idea",
    ".vscode",
    ".fleet",
    ".zed",
    ".junit.run",
    # JS / TS tooling state — package manager caches and hook state.
    ".husky",
    ".changeset",
    ".bun",
    ".deno",
...
# Dot-dirs that are INTENTIONALLY NOT in DEFAULT_EXCLUDE_DIR_NAMES because
# they carry architecturally relevant content (CI workflows, dev env
# description, toolchain config). Phase 133b dot-dir policy research §7.2:
# these fall through the walker by design.
#
#   .github/         — CI workflows, code-of-conduct, contribution guide
#                      (universal — present in 20 / 23 sample repos).
#   .gitlab/         — GitLab CI config (same role as .github/).
#   .azure/          — Azure DevOps pipelines.
#   .circleci/       — CircleCI config (config.yml).
#   .devcontainer/   — Dev Container / Codespaces dev-env description.
#   .cargo/          — Rust toolchain config (config.toml, vendor).
#   .mvn/            — Maven wrapper config.
#   .gradle/         — partly cache (DEFAULT_EXCLUDE_DIR_NAMES has it
#                      above), but `gradle/` (no dot) is wrapper config.
#
# If you add a new dot-dir convention to the trace exclude catalog, also
# decide whether it belongs in this "kept" list with a one-line reason.
