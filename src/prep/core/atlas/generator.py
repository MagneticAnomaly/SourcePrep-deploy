...
from prep.core.context_config import PipelineTask, compute_module_cap, compute_optimal_settings
from prep.core.llm_client import TASK_MAX_CHARS, _is_cloud_endpoint
...
class CodebaseAtlas:
    ...
    def _hub_str_with_evidence(
        ...
        from prep.core.git_evidence import atlas_decoration_enabled
        if not atlas_decoration_enabled() or self.project_root is None:
            return ", ".join(f"{p} ({d} edges)" for p, d in hubs)

        try:
            from prep.services.git_evidence_service import get_git_evidence
